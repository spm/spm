function [mri] = read_mri2(fn, s, offset);

% READ_MRI2 reads all MRI slices of size [Nx,Ny] from a 'int16' binary file
%
...
if fid~=-1
  ...
  fseek(fid, s(1)*s(2)*offset*2, 'bof');
  ...
  while feof(fid)==0 & i<s(3)
    ...
    [buf, count] = fread(fid, s([1, 2]), 'int16');
    if size(buf) == s([1, 2])
