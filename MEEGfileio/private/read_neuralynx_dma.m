...
% since modifying our hardware from 256 to 32 channel (19 March 2007), the number of channels in a DMA log file can vary
% start with the assumption that the file is from the 256 channel system with 8 boards that we have in Nimegen
nboards    = 8;
nchanboard = 32;
...
if needhdr
  ...
  if hdroffset
    ...
    hdr.orig       = neuralynx_getheader(filename);
  end
  ...
  fseek(fid, hdroffset + datoffset, 'bof');
  buf = fread(fid, blocksize, 'int32=>uint32');
  end_tsh = buf(4);
  end_tsl = buf(5);
  fclose(fid);
