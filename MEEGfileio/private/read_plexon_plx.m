...
hdr.Comment             = fread(fid, [1 128], 'char=>char'  );     % User-supplied comment
...
hdr.Name             = fread(fid, [1 32],  'char=>char'   );  % Name given to the DSP channel
hdr.SIGName          = fread(fid, [1 32],  'char=>char'   );  % Name given to the corresponding SIG channel
...
hdr.Comment          = fread(fid, [1 128], 'char=>char'   );
hdr.Padding          = fread(fid, 11,      'int32'  );

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function hdr = PL_EventHeader(fid)
hdr.Name          = fread(fid, [1 32],   'char=>char'  ); % name given to this event
...
hdr.Name          = fread(fid, [1 32],  'char=>char'  ); % name given to this channel
