% function spm_fmri_mancova_bp
% MAN[C]OVA and CVA for fMRI time-series
% FORMAT spm_fmri_mancova_bp
%___________________________________________________________________________
%
% AS FOR SPM_FMRI_MANCOVA BUT USES BAND-PASS FILTERING AS OPPOSED TO
% TEMPORAL SMOOTHING
%
% spm_fmri_mancova_bp effects a multivariate analysis of fMRI time-series that
...
% get Repeat time
%----------------------------------------------------------------------------
RT     = spm_input('Repeat time {secs}',5);


% high pass filter
%----------------------------------------------------------------------------
if spm_input('band pass filter',6,'yes|no',[1 0])
	u     = [1:q/2];
	u     = find((u < (q*RT)/60) | (u >= (q*RT)/8));
	for v = 1:length(u)
		d = sin(pi*[1:q]*u(v)/q);
		G = [G d(:)];
		d = cos(pi*[1:q]*u(v)/q);
