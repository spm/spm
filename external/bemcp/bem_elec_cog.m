function Lel = bem_elec_cog(electr,head_model,dipoles)

% Lel = bem_elec_cog(electr,[head_model,dipoles])
%
...
C11st = bem_Cii_lin(tsph1.tri,tsph1.vert,weight,defl) ;
weight = (sig2-sig3)/((sig1+sig2)*2*pi) ;
C12 = bem_Cij_lin(tsph1.vert,tsph1.tri,tsph2.vert,tsph2.tri,weight,defl) ;

disp('flag 2.3')
weight = (sig1-sig2)/((sig2+sig3)*2*pi) ;
C21 = bem_Cij_lin(tsph2.vert,tsph2.tri,tsph1.vert,tsph1.tri,weight,defl) ;
weight = (sig2-sig3)/((sig2+sig3)*2*pi) ;
C22st = bem_Cii_lin(tsph2.tri,tsph2.vert,weight,defl) ;

disp('flag 2.4')
weight = (sig1-sig2)/(sig3*2*pi) ;
C31 = Perm_r*bem_Cij_lin(tsph3.vert,tsph3.tri,tsph1.vert,tsph1.tri,weight,defl) ;
weight = (sig2-sig3)/(sig3*2*pi) ;
C32 = Perm_r*bem_Cij_lin(tsph3.vert,tsph3.tri,tsph2.vert,tsph2.tri,weight,defl) ;
...
C13 = bem_Cij_lin(tsph1.vert,tsph1.tri,tsph3.vert,tsph3.tri,weight,defl)*Perm_c ;
weight = sig3/((sig2+sig3)*2*pi) ;
C23 = bem_Cij_lin(tsph2.vert,tsph2.tri,tsph3.vert,tsph3.tri,weight,defl)*Perm_c ;
weight = 1/(2*pi) ;
C33st = Perm_r*bem_Cii_lin(tsph3.tri,tsph3.vert,weight,defl)*Perm_c ;
...
weight = 1/(2*pi*sig3) ;
G3 = Perm_r * bem_Gi_cog(tsph3.vert,tsph3.tri,dsph.dip,weight)' ;
