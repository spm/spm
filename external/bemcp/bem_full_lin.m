function L = bem_full_lin(head_model,dipoles)

% L = bem_full_lin([head_model,dipoles])
%
...
if nargin==0
    % Create simple 3-sphere model
%n = 22 ; n1 = 32 ; n2 = n ; n3 = n ;
%n = 32 ; n1 = 44 ; n2 = n ; n3 = n ;
	n1 = n ; n2 = n ; n3 = n ;
%	n1 = 2*round(.71*n) ; n2 = n ; n3 = n ;
...
C11st = bem_Cii_lin(tsph1.tri,tsph1.vert,weight,defl1,tsph1.pt4.vert) ;
weight = (sig2-sig3)/((sig1+sig2)*2*pi) ;
C12 = bem_Cij_lin(tsph1.vert,tsph2.vert,tsph2.tri,weight,defl2) ;
weight = sig3/((sig1+sig2)*2*pi) ;
C13 = bem_Cij_lin(tsph1.vert,tsph3.vert,tsph3.tri,weight,defl3) ;

disp('flag 2.3')
weight = (sig1-sig2)/((sig2+sig3)*2*pi) ;
C21 = bem_Cij_lin(tsph2.vert,tsph1.vert,tsph1.tri,weight,defl1) ;
weight = (sig2-sig3)/((sig2+sig3)*2*pi) ;
C22st = bem_Cii_lin(tsph2.tri,tsph2.vert,weight,defl2,tsph2.pt4.vert) ;
weight = sig3/((sig2+sig3)*2*pi) ;
C23 = bem_Cij_lin(tsph2.vert,tsph3.vert,tsph3.tri,weight,defl3) ;

disp('flag 2.4')
weight = (sig1-sig2)/(sig3*2*pi) ;
C31 = bem_Cij_lin(tsph3.vert,tsph1.vert,tsph1.tri,weight,defl1) ;
weight = (sig2-sig3)/(sig3*2*pi) ;
C32 = bem_Cij_lin(tsph3.vert,tsph2.vert,tsph2.tri,weight,defl2) ;
weight = 1/(2*pi) ;
C33st = bem_Cii_lin(tsph3.tri,tsph3.vert,weight,defl3,tsph3.pt4.vert) ;
