function [D] = spm_eeg_invert_noscale(D,Ip,fband,Nm,Nt,SmthInit,timewindow,SHUFFLELEADS)
...
% $Id: spm_eeg_invert_noscale.m 5243 2013-02-05 17:05:32Z gareth $
...
if isempty(SmthInit),
    SmthInit    = 0.6;				% Smoother factor for graph Laplacian
end;
Np   = length(Ip);				% Number of priors/3 for GS, ARD, MSP
hpf  = fband(1);				% high pass filter frequency (Hz)
lpf  = fband(2);				% low pass filter frequency (Hz)
...
if isempty(timewindow)
    ...
    w=1000*[timewindow(1) timewindow(2)];
end;
...
%----------------------------------------------------------------------
 ...
 if isfield(D.inv{val}.inverse,'Han'),
     if D.inv{val}.inverse.Han==0,
         W=1;
         disp('Not using Hanning window');
     end;
 end;
...
inverse.smooth = SmthInit;                    % smoothness (0 - 1)
