...
%
% Volume level:
%         (i) Maxima - tabulates p-values and statistics for entire volume
%                                               - see spm_list.m
%        (ii) Write - write out thresholded SPM as image
%                                               - see spm_write_filtered.m
%       (iii) Render - render on a (previously extracted) cortical surface
%                                               - see spm_render.m
%
% Cluster level:
%         (i) Maxima - tabulates p-values and statistics for nearest cluster
%                     ( Note that the cursor is repositioned to the  )
%                     ( nearest suprathreshold voxel by this option. )
%                                               - see spm_list.m
%
%        (ii) Small Volume Correction (S.V.C) tabulates p-values corrected
%             for a small specified volume of interest centred on the current
%             voxel. (Tabulation by spm_list.m)
%                                               - see spm_VOI.m
%       (iii) 
%
% Voxel level:
...
%
% FORMAT H = spm_results_ui('DrawButts',hReg,DIM,SPM,Finter,WS,FS)
% Draw GUI buttons
% hReg    - handle of XYZ registry object
% DIM     - 3 vector of image X, Y & Z dimensions
...
% H       - handles to frames enclosing buttons
%         - [hFvolB,hFclusB,hFvoxB,hConB]
...
%            hFspmB  - handle to frame enclosing SPM buttons
%            hFconB  - handle to frame enclosing control buttons
%
...
%=======================================================================
% H = spm_results_ui('DrawButts',hReg,DIM,Finter,WS,FS);
%                      
...
%-----------------------------------------------------------------------
hFvolB = uicontrol(Finter,'Style','Frame','Position',[010 130 230 030].*WS);
...
uicontrol(Finter,'Style','PushButton','String','maxima','FontSize',FS(10),...
	...
	'Callback','spm_list(''List'',SPM,VOL);',...
...
% uicontrol(Finter,'Style','PushButton','String','multiplane',...
%	'FontSize',FS(10),...
...
%-----------------------------------------------------------------------
hFclusB = uicontrol(Finter,'Style','Frame','Position',[010 090 230 030].*WS);
...
uicontrol(Finter,'Style','PushButton','String','maxima','FontSize',FS(10),...
	'ToolTipString',...
	'tabulate p-values & statistics for local maxima of current cluster',...
	'Callback','spm_list(''ListCluster'',SPM,VOL);',...
	'Interruptible','on','Enable','on',...
...
%-----------------------------------------------------------------------
hFvoxB = uicontrol(Finter,'Style','Frame','Position',[010 050 230 030].*WS);
...
%-SPM area
%-----------------------------------------------------------------------
hFspmB = uicontrol(Finter,'Style','Frame','Position',[245 090 145 070].*WS);
% uicontrol(Finter,'Style','Text','String','SPM results...',...
% 	'Position',[255 152 085 016].*WS,...
% 	'FontName',PF.times,'FontWeight','Normal','FontAngle','Italic',...
% 	'FontSize',FS(10),...
% 	'HorizontalAlignment','Left',...
% 	'ForegroundColor','w')
uicontrol(Finter,'Style','Text','String','SPM',...
	'Position',[250 095 135 060].*WS,...
...
%-ResultsUI controls
%-----------------------------------------------------------------------
hFconB = uicontrol(Finter,'Style','Frame','Position',[245 050 145 030].*WS);
uicontrol(Finter,'Style','Text','String','results controls',...
	'Position',[255 072 090 016].*WS,...
	'FontName',PF.times,'FontWeight','Normal','FontAngle','Italic',...
	'FontSize',FS(10),...
	'HorizontalAlignment','Left',...
	'ForegroundColor','w')
...
hExit  = uicontrol(Finter,'Style','PushButton','String','exit',...
	...
	'Interruptible','on','Enable','on',...
	'Position',[297 055 040 020].*WS);
hHelp  = uicontrol(Finter,'Style','PushButton','String','help',...
	'ToolTipString','results section help',...
	'FontSize',FS(9),'ForegroundColor','g',...
	'Callback','spm_help(''spm_results_ui'')',...
	'Interruptible','on','Enable','on',...
	'Position',[342 055 040 020].*WS);

varargout = {[hFvolB,hFclusB,hFvoxB,hFspmB,hFconB]};
