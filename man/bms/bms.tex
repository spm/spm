...
& \bf{.odds\_ratio} & posterior odds ratio, $\pi_{OCC}$, (number of models in Occam’s window, \\
& & default = 0). \\
...
& &  [1 $\times$ nm]. \\
& \bf{.theta} & samples from the parameter posterior density, $p(\theta|Y)$, [np $\times$ nsamp]; \\
& \bf{.theta\_sbj} & samples from the parameter posterior density, $p(\theta|Y)$, for each subject,\\
& &  [np $\times$ nsub $\times$ nsamp]; \\
& \bf{.mtheta\_sbj} & mean parameters for each subject, [np $\times$ nsub]; \\
& \bf{.stheta\_sbj} & standard deviation of the parameters for each subject [np $\times$ nsub]; \\
...
& \bf{.post} & model posterior probabilities for each subject, $p(m|Y)$, [nsub $\times$ nm]. \\
& \bf{.ma}	 & mean DCM.a parameters, [dima]. \\
& \bf{.mb}	 & mean DCM.b parameters, [dimb]. \\	
& \bf{.mc}	& mean DCM.c parameters, [dimc]. \\
& \bf{.md}	& mean DCM.d parameters, [dimd]. 
\end{tabular}\\\\
