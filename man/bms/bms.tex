<<<<<<< .mine
\chapter{Bayesian Model Inference \label{Chap:bms}}
...
We illustrate the methods using a DCM for fMRI study of the languange system.
...
\begin{figure}
\begin{center}
\includegraphics[width=100mm]{bms/Slide1.JPG}
...
Now open SPM and in the Menu window go to Batch, SPM, Stats, Bayesian Model Selection, BMS:DCM. This will open SPM's batch editor. Select an appropriate directory (eg. where you unzipped the archive), highlight Load model space and select the \verb!model_space.mat! file.
...
\begin{figure}
\begin{center}
\includegraphics[width=150mm]{bms/Slide2.JPG}
...
\begin{figure}
\begin{center}
\includegraphics[width=150mm]{bms/Slide3.JPG}
...
The group mean DCM parameters can be easily obtained from the MATLAB command window by loading the \verb!BMS.mat! file and then typing: \verb!mean_a=mean(BMS.DCM.ffx.bma.a,3)!. For the b, c and d parameters use the same code: \verb!mean_b=mean(...b,4)!, \verb!mean_c=mean(...c,3)! and \verb!mean_d=mean(...d,4)!. The subject specific mean DCM parameters can be obtained as follows: \verb!mean_as=mean(...as,4)!, \verb!mean_bs=mean(...bs,5)!, \verb!mean_cs=mean(...cs,4)! and \verb!mean_ds=mean(...ds,5)!. For random-effects change \verb!ffx! to \verb!rfx!.

\begin{figure}
\begin{center}
\includegraphics[width=150mm]{bms/Slide5.JPG}
...
\begin{figure}
\begin{center}
\includegraphics[width=150mm]{bms/Slide6.JPG}
...
& \bf{.as} & same as bma.a but for each subject [dima $\times$ nsub $\times$ nsamp]. \\
& \bf{.bs} & same as bma.b but for each subject [dimb $\times$ nsub $\times$ nsamp]. \\
& \bf{.cs} & same as bma.c but for each subject [dimc $\times$ nsub $\times$ nsamp]. \\
& \bf{.ds} & same as bma.d but for each subject [dimd $\times$ nsub $\times$ nsamp]. \\
& \bf{.Ep} & samples from posterior density over parameters (vector) [np $\times$ nsamp]. \\
\end{tabular}\\\\
...
The group mean DCM parameters can be easily obtained from the MATLAB command window by loading the \verb!BMS.mat! file and then typing: \verb!mean_a=mean(BMS.DCM.ffx.bma.a,3)!. For the b, c and d parameters use the same code: \verb!mean_b=mean(...b,4)!, \verb!mean_c=mean(...c,3)! and \verb!mean_d=mean(...d,4)!. 

The subject specific mean DCM parameters can be obtained as follows: \verb!mean_as=mean(...as,4)!, \verb!mean_bs=mean(...bs,5)!, \verb!mean_cs=mean(...cs,4)! and\newline \verb!mean_ds=mean(...ds,5)!. For random-effects change \verb!ffx! to \verb!rfx!. 

If we are interested in the modulatory connection from region 1 to region 3 (that is modulated by the second input), then the mean value of this for 
 Subject 10 is given by \verb!mean_bs(3,1,2,10)! (which should be 0.7475).
The mean connection values for all subjects can be gotten with the matlab 
syntax $\verb!squeeze(mean_bs(3,1,2,:))!.

These subject specific mean parameters can then act as summary statistics for 
a standard group analysis. For example to look for significant differences 
between eg. a control group and a patient group in a modulatory parameter one would implement a two-sample t-test on data from the appropriate entries in the \verb!mean_bs! matrices. Similarly, if one has 3 groups one would use a 3-level ANOVA.
