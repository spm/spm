...
\\
\\
Choose the \textsc{Prepare} tool from the ``Other'' drop-down menu and select the \texttt{Mspm8\_subject1.mat} file. This will make several menus available in the lower left SPM window. Select \texttt{Sensors} $\rightarrow$ \texttt{Load EEG sensors} $\rightarrow$ \texttt{Assign default} from the menu. This will tell SPM to assume default locations for the BioSemi channels. Chapter \ref{Chap:eeg:preprocessing} describes in detail how you can use the other options to use digitized sensor location data. Then select the ``Save'' option from the ``File'' menu. Note that if you leave out the ``Save'' step, no information will be written to the \texttt{mat}-file and you won't be able to display the data properly later on.
