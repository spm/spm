...
\section{DCM for Induced Responses}
...
\begin{figure}
\begin{center}
\includegraphics[width=100mm]{multimodal/figures/irfigure1}
...
\subsubsection{load, save, select model type}
...
1 0
\end{verbatim}
in first row of the box, which means that there will be some additive modulation of connections that we will define later for ``faces''  (some coefficient multiplied by 1) and this modulation will not be there for ``scrambled'' (the same coefficient multiplied by 0). If we now click somewhere outside the box, a default name will be assigned to this effect - ``effect1''. It will appear in the small text boxt next to the coefficients box. It is possible to change this name to something else e.g. ``face''.
...
\begin{figure}
\begin{center}
\includegraphics[width=160mm]{multimodal/figures/irfigure2}
...
\subsubsection{Electromagnetic model}
...
\begin{figure}
\begin{center}
\includegraphics[width=160mm]{multimodal/figures/irfigure3}
...
\begin{figure}
\begin{center}
\includegraphics[width=60mm]{multimodal/figures/irfigure4}
...
This will display the coupling matrices representing the coupling strength from source to target frequencies. These matrices are obtained by multiplying the between-mode matrices (estimated by the inversion) with the frequency profiles of the modes (see \cite{cc_induced}). The arrangement of the matrices corresponds to arrangements of the buttons in the connectivity matrices explained above.

\subsubsection{Coupling (B-Hz)}
This presentation of results is similar to the previous coupling matrices but this time shows modification of coupling by condition (eg. in our example it shows which frequency couplings are different for faces as opposed to scrambled faces). 
...
comprising the MRI, CoReg, Forward, Invert and Save sub-steps.
...
\begin{figure}
\begin{center}
%\includegraphics[width=50mm]{multimodal/figures/endogenous}
