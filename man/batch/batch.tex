...
\item[Error reporting] If a batch fails to complete, a standardised report
  will be given in the \matlab\ command window. When running a batch from the
  GUI, this can be saved to an error \verb|.mat| file. When running a batch
  from the command line, such a file will be created always. It will be
  located in the current MATLAB directory and named
