...
chapter~\ref{ch:face_data}). To follow this tutorial, it is not necessary to
...
details are described in chapter~\ref{ch:face_data}.
...
parameters) as described in chapter~\ref{ch:face_data}. Do not enter any data that
...
In chapter~\ref{ch:face_data}, each processing step was performed on its own. In
...
\begin{figure}[htbp]
  \centering
  \includegraphics[height=.3\textheight]{batch/batch_template_single_subject_nodeps}

  \includegraphics[height=.3\textheight]{batch/batch_template_single_subject}
