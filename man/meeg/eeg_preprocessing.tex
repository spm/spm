...
one would use one of the source reconstruction techniques or statistical
...
system. We recommend beginners use the GUI first, because this will
prompt SPM to ask for all relevant information needed to process
the data. The batch input system is designed for repetitive analyses
of data (eg. from multiple subjects) once the user knows what should be done, and in which order. The
...
provide some sample batch scripts (\textit{histexample.m}) in the
...
or datasets which contain information about trials. If 'define'
...
in the Matlab workspace. Then you have two options depending on whether
you would be willing to use a Matlab script or want to only use the GUI.
If you want to only use the GUI, we suggest that you assemble your dataset
in EEGLAB (http://sccn.ucsd.edu/eeglab/) and then  save it and convert
to SPM as any other format. The developers of EEGLAB have invested a lot of effort
...
\item .trial - cell array of trials containing matrices with identical dimensions (channels x time). 
\item .time - cell array of time vectors (in sec) - one cell per trial, containing a time vector the same length as the
...
If your data only has one trial (e.g. it is already an average or it is raw continuous data) 
you should only have one cell in .trial and .time fields of the raw struct.
...
Here are the reasons why we changed the format. Previously, we placed
...
this in the future, we have introduced two changes. The first is that
SPM8 now
...
information available to fix the inconsistency, it will be fixed on the fly.
For instance, you will usually see some messages about missing information when
...
remove trials. There is no longer a need to update a variable for the number of trials. Also there is no need to check for the presence of particular fields
in the struct in every SPM function. All the methods are guaranteed to work on
a valid object. There are many more simplifications along these lines due to the use of
`objects' and `methods'. In summary, the change in format has resulted in a
...
from the various data formats. In some cases it can also supplement
the converted dataset with information not directly present in the raw data.
For instance, SPM can recognize common EEG setups (extended 1020, Biosemi, EGI)
...
locations in an experiment. In these cases, further information needs
...
submenu. The 'Channel types' submenu allows reviewing and
changing the channel types. Use the 'Review' option to examine the
presently set channel types. During conversion, SPM  will make an informed 'guess' at the correct channel types but this can sometimes go
wrong, especiallly for EEG data. To set a particular channel group
...
and cannot be modified using the GUI.
...
image are hard-wired in SPM code. So if you provide the coordinates of these specific
...
SPM file using the 'Load EEG sensors' menu. There are 3 options:
...
\item 'From a *.mat file' - this option is for the kind of files that
  ...
  trying to get them into one of the standard formats. SPM will ask for two
  files. The sensors file should contain an $N \times 3$ matrix,
  ...
  The fiducials file should contain a $K \times 3$ matrix, where $K$
  (usually 3) is the number of fiducials.  You will then be asked to provide
...
clear anatomical landmarks. In this case there should be an additional file measured with a Polhemus-like
...
handles this situation is in two steps. First, this additional file is converted into the same
...
shape.pnt - a $K \times 3$ matrix (possibly empty) with headshape points i.e. points that are on the surface of the head
...
shape.fid.label - $M \times 1$ cell array of strings with the labels of the points in shape.fid.pnt. As mentioned
 above, $M$ should be at least 3 for the coregistration to work.
...
is implemented using the 'Coregister' option. For details of how this option works
...
automatically. In case of invoking prep from the reviewing tool you should
press the 'OK' button that will appear at the bottom left of the interactive window, and
...
however, SPM will use 2D-coordinates laid out in a topographically 
unmeaningful rectangular pattern.
...
for multiple versions of a given channel name. Case can be
...
must be $\geq 0.95$. \\
{\bf Rxy} & & - & A factor that determines the ratio of the width of the display
...
The SPM8 distribution includes the latest version of the 'Fieldtrip' toolbox
...
version in the Matlab path from the one used by SPM, SPM will only use its own
...
identical to the 'Just read' option in the GUI. The SPM-format stores the
...
data linked in, and SPM functions won't know what to do with this
...
the events that occured within that trial or possibly in some time window around it
...
encounter errors in Fieldtrip functions. Please report such errors on the
SPM or Fieldtrip mailing list so that we can fix them. ftraw(0) is
...
information available in the raw data (e.g. when you have an external stimuli file),
you should use the first.
\\
For the first method, you specify a $N \times 2$ so-called \textit{trl}-matrix, where
each row contains the start and end of a trial (in samples). Optionally, there can be a third column
...
For the second method, one should define the pre- and post-stimulus interval, and also specify
...
same label - in the GUI, just select several events using Shift or Ctrl key.

For both methods of input you also have to set a (0/1)-flag (no/yes) indicating whether you
want to review the information for all trials after selecting them. This allows you 
to make sure that all your trials are there. You should set the review-flag to 0,
if you write a non-interactive script. In the GUI you can review a list of the epochs you defined
...
(implemented by other software) and supply that information to
SPM.

Thresholding of the data is implemented in two passes. In the first, SPM
...
absolute data surpasses the threshold in some channel (excluding bad
...
This approach estimates weights, lying between 0 and 1, that indicate how 
...
to specify the width of a smoothing kernel for the residuals. As
...
to use a GUI or read information from a file to specifying the montage.
...
row contains weights for how the old channels must be weighted to produce new
channels in the montage. On the right hand side, you see a graphical representation
...
ask an expert or write to the SPM mailing list. The specification will only need to be done once for your setup and then you can
save the montage and use it routinely. After changing the weights of the matrix, you can check the montage by pressing
...
not involved in the montage. For instance, if you the apply montage defined for your EEG channels but there are also
...
script, by default, when you don't specify an output filename,
...
use the display tool on the concatenated file to be able to display in the same graph
data coming from different files. This is the
...
files, one containing the instantaneous power and the other, optionally
...
\textit{spm\_eeg\_average\_TF}. The prepended output letter is 'm'.
...
trial types in the file. Note that SPM will zero-pad this vector
 (or matrix) if you specify fewer contrast weights than you have trials.
...
\item{\textbf{source}}
display source reconstructions (if any). See below (2- source reconstructions visualization).
\end{itemize}
...
An additional axis (bottom right) provides the user with the temporal and horizontal scale of the displayed data).
...
Buttons 8 and 9 allow the user to scroll through the data from marker to marker (backward and forward in time).
...
Each sub tab has a label given by the corresponding source reconstruction comment which is specified by the user when source reconstructing the data (see relevant section in the SPM manual).
...
The top part of the window displays a rendering of the reconstruction on the cortical surface that has been used. User can scroll through peri-stimulus time by using the temporal slider below the rendered surface. Other sliders allow the user to (i) change the transparency of the surface (left slider) and (ii) threshold the colormap (right sliders).
 In the center, a butterfly plot of the reconstructed intensity of cortical source activity over peri-stimulus time is displayed.
...
SPM EEG REVIEW allows quick and easy switching between different models and trials, for a visual comparison of cortical source activities.
...
Although all functions can be called by the GUI, using only the GUI for preprocessing multi-subject data is a cumbersome
and error-prone affair. For this reason, we included functions
in SPM8 which should make batching jobs
feasible, with only a small amount of matlab knowledge required.
...
This method uses the new SPM8 batch system described in chapter \ref{Chap:batch_interface}.
...
To do this, you first have to analyze one data set using the
GUI or batch system. In SPM8, whenever a preprocessing
...
the script already has all the input arguments which you gave
...
you are asked for the file from which you want to extract the history
...
the analysis, there is no way to do this via the batch system. In the
