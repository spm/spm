...
\item{Highlight `Directory' and select the \verb!DIR/design! directory you created earlier.}
\item{Save the job as \verb!specify.mat! and press `RUN'}
\ei
SPM will then write an \verb$SPM.mat$ file to the 
\verb!DIR/design! directory. It will also plot the design
matrix, as shown in Figure~\ref{aud_design}. 
\begin{figure}
\begin{center}
\includegraphics[width=60mm]{design}
\caption{\em Design matrix. \label{aud_design}}
\end{center}
\end{figure}
...
file saved in the design subdirectory}
\item{Highlight the `Directory' option and then choose the 
\verb!DIR\classical! directory}
...
In Figures~\ref{aud_slices}\ref{aud_sections}\ref{aud_render} the `active $>$ rest' activation has been superimposed on the spatially normalised, bias-corrected 
