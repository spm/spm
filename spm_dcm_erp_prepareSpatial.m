...
% Stefan Kiebel
% $Id: spm_dcm_erp_prepareSpatial.m 710 2006-12-21 14:59:04Z stefan $
...
if DCM.options.Spatial_type == 1
    ...
    if strcmpi('pol', spm_str_manip(sensorfile, 'e'))
        ...
        try
            ...
            name  = fieldnames(tmp);
            sens  = getfield(tmp,name{1});
            
            Fname_fid = 'H:\projects\collaborations\Virginie\MNI_FID'
            tmp   = load(Fname_fid, '-mat');
            name  = fieldnames(tmp);
            fid   = getfield(tmp,name{1});;
        catch
            try
                DCM.M.dipfit.vol;
                return
    ...
    load(DCM.M.Dfile);
    figure, text(sensreg(:, 1), sensreg(:,2), sensreg(:,3), D.channels.name(1:length(sensreg)))
