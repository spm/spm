function def = spm_uw_get_def(Bx,By,Bz,beta)
% Compute deformation field from coefficient vector
% FORMAT [def] = spm_uw_get_def(Bx,By,Bz,beta)
% Bx, By, Bz   - Separable basis sets such that B=kron(Bz,kron(By,Bx));
% beta         - Coefficient vector for basis set.
%
% FORMAT [def] = spm_uw_get_def(dim,order,beta)
% dim          - Dimensionality of image in x,y and z directions.
% order        - Order of DCT set in x,y and z directions.
% beta         - Coefficient vector for basis set.
%__________________________________________________________________________
%
% Compute the equivalent to kron(Bz,kron(By,Bx))*beta in a faster and more
% efficient way. Note that this routine can be used to calculate AtY
% efficiently as well since:
...
if nargin == 4
   ...
   if mx*my*mz ~= size(beta,1)
      error('Size mismatch between beta and basis-set.');
   end
elseif nargin == 3
   if numel(Bx) ~= 3 || numel(By) ~= 3
      error('Wrong dimensionality on input.');
