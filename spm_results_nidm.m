...
if ~isempty(SPM.xM.VM)
    ...
    p.entity(idMask2,{...
        ...
        'nidm:inCoordinateSpace',id_emask_coordspace,...
...
if isfield(SPM.xVi,'form')
    ...
    if ~isfield(SPM.xVi,'Vi') || numel(SPM.xVi.Vi) == 1 % assume it's identity
        ...
        extra_fields_NM = {...
            'nidm:hasErrorDependence','nidm:ArbitrarilyCorrelatedError',...
            'nidm:dependenceSpatialModel','nidm:SpatiallyGlobalModel',...
...
p.entity(idMask1,{...
    ...
    'nidm:inCoordinateSpace',id_data_coordspace,...
    'crypto:sha512',{sha512sum(spm_file(files.mask,'cpath')),'xsd:string'},...
    });
id = originalfile(p,SPM.VM.fname,idMask1,'nidm:MaskMap');
...
for i=1:numel(SPM.Vbeta)
    ...
    id = originalfile(p,SPM.Vbeta(i).fname,idBeta{i},'nidm:ParameterEstimateMap');
...
id = originalfile(p,SPM.VResMS.fname,idResMS,'nidm:ResidualMeanSquaresMap');
...
id = originalfile(p,SPM.xVol.VRpv.fname,idRPV,'spm:ReselsPerVoxelMap');
...
for c=1:numel(xSPM.Ic)
    ...
    id = originalfile(p,SPM.xCon(xSPM.Ic(c)).Vspm.fname,idSPM{c},'nidm:StatisticMap');
    ...
    if xSPM.STAT == 'T'
        ...
        id = originalfile(p,SPM.xCon(xSPM.Ic(c)).Vcon.fname,idContrast,'nidm:ContrastMap');
...
for i=1:numel(files.dmask)
    ...
    idDMask = getid(['niiri:display_map_id' postfix],isHumanReadable);
    p.entity(idDMask,{...
        ...
        'nidm:inCoordinateSpace',currCoordSpace,...
        'crypto:sha512',{sha512sum(spm_file(files.dmask{i},'cpath')),'xsd:string'},...
        });
    id = originalfile(p,files.dmask{i},idDMask,'nidm:DisplayMaskMap');
