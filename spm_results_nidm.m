...
underlayFile = ''; %- fullfile(spm('Dir'),'canonical','avg152T1.nii')
...
%-Underlay file (as NIfTI)
%--------------------------------------------------------------------------
if ~isempty(underlayFile)
    files.underlay = fullfile(outdir,['underlay.nii' gz]);
    img2nii(underlayFile,files.underlay);
else
    files.underlay = '';
end
...
if ~isempty(SPM.xM.VM)
    ...
    p.entity('niiri:mask_id_2',{...
        ...
        'nidm:coordinateSpace',id_emask_coordspace,...
        'crypto:sha',{sha512sum(spm_file(files.emask,'cpath')),'xsd:string'},...
        });
end

%-Activity: Model fitting
%==========================================================================
p.activity('niiri:model_fitting_id',{...
    'prov:type','nidm:ModelFitting',...
    'prov:label','Model fitting',...
    });
p.wasAssociatedWith('niiri:model_fitting_id', 'niiri:software_id');
p.used('niiri:model_fitting_id', 'niiri:design_matrix_id');
p.used('niiri:model_fitting_id','niiri:data_id');
if ~isempty(SPM.xM.VM)
    p.used('niiri:model_fitting_id','niiri:mask_id_2');
end
...
p.entity('niiri:mask_id_1',{...
    ...
    'nidm:coordinateSpace',id_data_coordspace,...
    'crypto:sha',{sha512sum(spm_file(files.mask,'cpath')),'xsd:string'},...
    });
p.wasGeneratedBy('niiri:mask_id_1','niiri:model_fitting_id');
...
for i=1:numel(SPM.Vbeta)
    if ~isempty(files.beta)
        extra_fields = {...
            ...
            'crypto:sha',{sha512sum(files.beta{i}),'xsd:string'},...
        };
    else
        extra_fields = {};
    end
    ...
    p.wasGeneratedBy(sprintf('niiri:beta_map_id_%d',i), 'niiri:model_fitting_id');
end
...
p.entity('niiri:residual_mean_squares_map_id',{...
    ...
    'nidm:coordinateSpace',id_data_coordspace,...
    'crypto:sha',{sha512sum(spm_file(files.resms,'cpath')),'xsd:string'},...
    });
p.wasGeneratedBy('niiri:residual_mean_squares_map_id','niiri:model_fitting_id');
...
p.entity('niiri:resels_per_voxel_map_id',{...
    ...
    'nidm:coordinateSpace',id_data_coordspace,...
    'crypto:sha',{sha512sum(spm_file(files.rpv,'cpath')),'xsd:string'},...
    });
p.wasGeneratedBy('niiri:resels_per_voxel_map_id','niiri:model_fitting_id');
...
for c=1:numel(xSPM.Ic)
    ...
    p.entity(idConVec,{...
        ...
        'prov:type',['nidm:' xSPM.STAT 'Contrast'],...
    ...
    p.activity(idConEst,{...
        'prov:type','spm:ContrastEstimation',...
    ...
    p.entity(idSPM,{...
        ...
        'nidm:coordinateSpace',id_data_coordspace,...
        'crypto:sha',{sha512sum(files.spm{c}),'xsd:string'},...
        });
    ...
    if xSPM.STAT == 'T'
        ...
        p.entity(idContrast,{...
            ...
            'nidm:coordinateSpace',id_data_coordspace,...
            'crypto:sha',{sha512sum(spm_file(files.con{c},'cpath')),'xsd:string'},...
            });
        ...
        p.entity(idSE,{...
            ...
            'nidm:coordinateSpace',id_data_coordspace,...
            'crypto:sha',{sha512sum(spm_file(files.conse{c},'cpath')),'xsd:string'},...
            });
    ...
    if xSPM.STAT == 'F'
        ...
        p.entity(idESS,{...
            ...
            'nidm:coordinateSpace',id_data_coordspace,...
            'crypto:sha',{sha512sum(spm_file(files.ess{c},'cpath')),'xsd:string'},...
            });
...
if xSPM.STAT == 'T'
    st = [st, 'prov:type','nidm:InferenceOneTailed'];
end
...
for i=1:numel(files.fmask)
    ...
    p.entity(idFMask,{...
        ...
        'nidm:coordinateSpace',currCoordSpace,...
        'crypto:sha',{sha512sum(spm_file(files.fmask{i},'cpath')),'xsd:string'},...
        });
...
if ~isempty(files.svcmask)
    ...
    p.entity('niiri:svc_mask_map_id',{...
        'prov:type','nidm:MaskMap',...
        'prov:location',{uri(spm_file(files.svcmask,'cpath')),'xsd:anyURI'},...
        'prov:label',{'SVC Mask','xsd:string'},...
        'nidm:fileName',{spm_file(files.svcmask,'filename'),'xsd:string'},...
        'nidm:coordinateSpace',currCoordSpace,...
        'crypto:sha',{sha512sum(spm_file(files.svcmask,'cpath')),'xsd:string'},...
        });
    p.used('niiri:inference_id','niiri:svc_mask_map_id');
end
...
%--------------------------------------------------------------------------
if ~isempty(files.underlay)
    underlay = {'nidm:underlayFile',{uri(files.underlay),'xsd:anyURI'}};
else
    underlay = {};
end
p.entity('niiri:excursion_set_id',{...
    ...
    'nidm:coordinateSpace',id_data_coordspace,...
    'crypto:sha',{sha512sum(spm_file(files.tspm,'cpath')),'xsd:string'},...
    underlay{:},...
    });
p.wasGeneratedBy('niiri:excursion_set_id', 'niiri:inference_id'); 

%-Entity: Set Level Statistic
%--------------------------------------------------------------------------
if size(TabDat.dat,1) > 0
    p.entity('niiri:set_level_statistic_id',{...
    ...
    p.wasDerivedFrom('niiri:set_level_statistic_id','niiri:excursion_set_id');
end
...
for i=1:numel(idx)
    ...
    p.entity(['niiri:cluster_' iClus],{...
        ...
        'prov:label',{['Cluster Level Statistic: ' iClus],'xsd:string'},...
    ...
    p.wasDerivedFrom(['niiri:cluster_' iClus],'niiri:set_level_statistic_id');
end
...
for i=1:size(TabDat.dat,1)
    ...
    p.entity(['niiri:peak_' iPeak],{...
        ...
        'prov:label',{['Peak Level Statistic: ' iPeak],'xsd:string'},...
