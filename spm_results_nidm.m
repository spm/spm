...
% Guillaume Flandin
% $Id: spm_results_nidm.m 6585 2015-10-28 16:02:13Z guillaume $
...
%pp.add_namespace('stato','http://purl.obolibrary.org/obo/');
...
p.entity(idDesignMatrix,{...
    ...
    nidm_conv('nidm_regressorNames',p),{SPM.xX.name,'xsd:string'},...
    extra_fields_drift{:},...
    extra_fields_basis_set{:}
    });
    %nidm_conv('nidm:hasfMRIDesign',p),nidm_conv('nidm:BlockDesign',p),...
...
if ~isempty(SPM.xM.VM)
    ...
    p.entity(idMask2,{...
        ...
        nidm_conv('nidm:inCoordinateSpace',p),id_emask_coordspace,...
...
id = originalfile(p,SPM.VM.fname,idMask1,nidm_conv('nidm_MaskMap',p));
...
id = originalfile(p,SPM.VResMS.fname,idResMS,nidm_conv('nidm_ResidualMeanSquaresMap',p));
...
id = originalfile(p,SPM.xVol.VRpv.fname,idRPV,nidm_conv('nidm_ReselsPerVoxelMap',p));
...
for c=1:numel(xSPM.Ic)
    ...
    p.entity(idConVec,{...
        ...
        nidm_conv('nidm_contrastName',p),{SPM.xCon(xSPM.Ic(c)).name,'xsd:string'},... %esc
    ...
    id = originalfile(p,SPM.xCon(xSPM.Ic(c)).Vspm.fname,idSPM{c},nidm_conv('nidm_StatisticMap',p));
    ...
    if xSPM.STAT == 'T'
        ...
        id = originalfile(p,SPM.xCon(xSPM.Ic(c)).Vcon.fname,idContrast,nidm_conv('nidm_ContrastMap',p));
    ...
    if xSPM.STAT == 'F'
        ...
        p.entity(idESS,{...
            'prov:type',nidm_conv('nidm:ContrastExtraSumOfSquaresMap',p),...
        ...
        id = originalfile(p,SPM.xCon(xSPM.Ic(c)).Vcon.fname,idESS,nidm_conv('nidm:ContrastExtraSumOfSquaresMap',p));
...
p.entity(idSearchSpace,{...
    ...
    nidm_conv('spm_noiseFWHMInVoxels',p),{xSPM.FWHM,'xsd:string'},...
    nidm_conv('spm_noiseFWHMInUnits',p),{TabDat.ftr{7,2}(1:3),'xsd:string'},...
...
p.entity(idExcursionSet,{...
    ...
    nidm_conv('nidm_numberOfSignificantClusters',p),{c,'xsd:int'},...
...
for i=1:numel(idx)
    ...
    p.entity(idCluster{i},{...
        'prov:type',nidm_conv('nidm_SignificantCluster',p),...
...
if ~isempty(i)
    ...
    if nargin == 2
        qname = C{i,1};
        j = find(qname == ':');
        if ~isempty(j), qname = qname(j(end)+1:end); end
        p.add_namespace(C{i,2},['http://purl.org/nidash/nidm#' qname]);
    end
else
...
%==========================================================================
function nidm_store_constants
urlwrite('https://raw.githubusercontent.com/cmaumet/nidm/prefix_file/nidm/nidm-results/terms/prefixes.csv','prefixes.csv');
...
'nidm:NIDM_0000070', 'nidm_SignificantCluster';...
...
'nidm:NIDM_0000111', 'nidm_numberOfSignificantClusters';...
