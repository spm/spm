...
% MDP.C{G}(O,1)         - prior probabilities over final outcomes (log preferences)
...
try, eta   = MDP(1).eta;   catch, eta   = 128;  end % learnability
try, chi   = MDP(1).chi;   catch, chi   = 1/64; end % Occam window updates
try, tol   = MDP(1).tol;   catch, tol   = 32;   end % likelihood tolerance
...
for t = 1:T
            ...
            if ~MDP(m).o(g,t)
                ...
                if MDP(m).n(g,t) > 0
                    ...
                    if n == m
                        ...
                        %--------------------------------------------------
                        F             = spm_dot(spm_psi(a{m,g}),Q(m,j,t));
                ...
                elseif MDP(m).n(g,t) < 0
                    ...
                    %------------------------------------------------------
                    Fm{g,m} = spm_dot(spm_psi(a{m,g}),Q(m,j,t));
    ...
    for m = M(t,:)
        for g = 1:Ng(m)
            ...
            %--------------------------------------------------------------
            if MDP(m).n(g,t) < 0
                j  = 1:numel(MDP); j(m) = [];         % sensory attenuation
    ...
    for m = M(t,:)
        ...
        %==================================================================
        [G,Q,F] = spm_forwards(O,Q,A,K,C,H,W,t,T,min(T,t + N),m,id,tol);
...
% auxillary functions
%==========================================================================
function [G,P,F] = spm_forwards(O,P,A,B,C,H,W,t,T,N,m,id,tol)
...
%--------------------------------------------------------------------------
[Q,F]    = spm_VBX(O(m,:,t),P(m,:,t),A(m,:),id{m},tol);
P(m,:,t) = Q;
    ...
    for k = 1:size(B,3)                      % search over actions
        ...
        if u(k)                              % evaluating plausible paths
            ...
            for i = j(:)'
                ...
                %----------------------------------------------------------
                P(m,:,t + 1) = Q(:,k);
                E      = spm_forwards(O,P,A,B,C,H,W,t + 1,T,N,m,id,tol);
...
%     end
    ...
    for t = 1:T
        ...
        for g = 1:numel(qa)
            j  = id{m}.A{g};
            LL = spm_dot(spm_psi(qa{g}),O{m,g,t});
...
for i = 1:64
    ai        = a*i/32;
    ELOG(:,i) = spm_psi(ai);
    LOGE(:,i) = log(spm_dir_norm(ai));
end

subplot(2,2,1)
plot(LOGE),hold on,plot(ELOG,':'),hold off
title ('log expectation & expected log')
legend('log expectation')

subplot(2,2,2)
imagesc(1:32,(1:64)/32,ELOG')
title ('expected log')
