...
for t = 1:T
        ...
        if t > 1
            ...
            if OPTIONS.A
                ...
                for k = 1:size(J{m},1)
                    ...
                    F(k)  = 0;
                    for g = 1:Ng(m)
                        [j,i] = spm_get_edges(id{m},g,qs);
                        po    = spm_dot(MDP(m).A{g},qs(j));
                        F(k)  = F(k) - qo{i}'*spm_log(po);
                    end
                end
    ...
    for m = M(t,:)
        ...
        for i = ig{m}
            ...
            % domain of A{g}
            %--------------------------------------------------------------
            [j,g]  = spm_get_edges(id{m},i,MDP(m).s(:,t));
            ...
            if ~MDP(m).o(g,t)
                ...
                if MDP(m).n(g,t) > 0
                    ...
                    if n == m
                        ...
                        %--------------------------------------------------
                        F             = spm_log(spm_dot(A{m,i},Q(m,j,t)));
                ...
                elseif MDP(m).n(g,t) < 0
                    ...
                    %------------------------------------------------------
                    Fm{g,m} = spm_log(spm_dot(A{m,i},Q(m,j,t)));
...
for m = 1:size(MDP,1)
    ...
    for g = 1:Ng(m)
        for t = 1:T
            [j,i]         = spm_get_edges(id{m},g,Q(m,:,t));
            MDP(m).Y{i,t} = spm_dot(A{m,g},Q(m,j,t));
            MDP(m).j{g,t} = j;
            MDP(m).i{g,t} = i;
        end
    end
...
for v = 1:16
    ...
    for t = 1:T
        ...
        for g = ig(:)'
            [j,i]   = spm_get_edges(id{m},g,Q(m,:,t));
            qa{m,g} = qa{m,g} + spm_cross(O{m,i,t},Q{m,j,t});
    ...
    for t = 1:T
        ...
        for g = ig
            [j,i]  = spm_get_edges(id{m},g,Q(m,:,t));
            j  = unique(j,'stable');
            LL     = spm_dot(spm_norm(qa{m,g}),O{m,i,t});
            LL = spm_log(LL);
            %  = spm_dot(spm_psi(qa{m,g}),O{m,i,t});
...
% Check for constraints in latent space
%--------------------------------------------------------------------------
if isfield(id,'cid')
    cid   = id.cid;                           % disallowed states
    cid   = cid(hif,:);                       % in hif factors
    D     = true(Ns);
    for i = 1:size(cid,2)
        ind  = num2cell(cid(:,i));
        j    = sub2ind(Ns,ind{:});
