...
% https://sound-effects.bbcrewind.co.uk/search?cat=Nature
...
spm_wav2I = @(s,RGB)real(spm_wft(s,RGB.k,RGB.n));

% Crop and resize to 128 x128 pixels
%--------------------------------------------------------------------------
RGB.Fs = Fs;                                   % sample rate
RGB.n  = fix(Fs*32/1000);                      % window (ms)
RGB.k  = fix(linspace(40,4000,64)/(Fs/RGB.n)); % frequencies (Hz)
...
s = spm_iwft(I,RGB.k,RGB.n);
sound(s,RGB.Fs);

% image
%--------------------------------------------------------------------------
subplot(4,1,2)
imagesc(I), xlabel('time'), ylabel('frequency')
