...
try
    ...
    %----------------------------------------------------------------------
    a_o = @(x,v,P) (v - x.o(1))/P.m - x.o(2)/P.b + occ(v)*(x.x(1) - x.o(1))/P.a; % occulomotor
    a_x = @(x,v,P) (v - x.x(1))/P.m - x.x(2)/1;            % target
    a_a = @(x,a,P)            a/P.m - x.o(2)/P.b/2;        % action
    ...
    % first level
    %----------------------------------------------------------------------
    G(1).f  = @(x,v,a,P) [x.o(2); a_a(x,a,P); x.x(2); a_x(x,v,P)];
...
try
    ...
    xU.u  = -sin((1:N)*w);                      % sinusoidal target motion
end
