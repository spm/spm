function MDP = DEM_Tower
% Demo of planning as sophisticated inference (Tower of Hanoi)
%__________________________________________________________________________
%
% This demonstration routine illustrates planning as inductive
% inference in the context of the Tower of Hanoi paradigm.
...
% Transitions among arrangements are modelled as  controllable paths,
...
for c = 1:Nc                                  % source column
    for t = 1:Nc                              % target column
        ...
        for s = 1:Ns                          % for each arrangement
            ...
            g = find(a(:,t),1,'last');        % height of target column
...
% Specify pecise intentions, in terms of the end state or target (r)
%--------------------------------------------------------------------------
H{f}(r)  = 32;
...
mdp   = MDP;
mdp.N = 0;
mdp.T = 2*(Nb + 1);                       % maximum number of moves
NT    = 100;                              % number of trials
N     = [1 32];                           % precision of prefered end states
...
for t = 1:NT
    ...
    n     = randperm(2*Nb,1);             % random number of moves
    r     = spm_target(s,n,B{1});         % target configuration
    mdp.C = spm_cost(r,A,1/4);            % constraints on the path
    mdp.s = s;

    for n = 1:numel(N)

        % Inductive planning (intended target)
        %------------------------------------------------------------------
        mdp.H{1}(:) = 1;                  % Prior Dirichlet counts
        mdp.H{1}(r) = N(n);
...
xlabel('Inductive planning'), ylabel('success (%)'), box off, axis square
legend(str(2:end))
