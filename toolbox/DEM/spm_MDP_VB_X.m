...
for t = 1:T
    ...
    for m = 1:size(MDP,1)
        ...
        if ~HMM
            ...
            % sample state - based on previous action
            %--------------------------------------------------------------
            for f = 1:Nf(m)
                try
                    s{m}(f,t) = MDP(m).s(f,t);
...
%                   po        = spm_softmax(F*512);
    ...
    % Couple outcomes among multiple models
    %======================================================================
    if ~HMM
        for m = 1:size(MDP,1)
            for g = 1:Ng(m)
                
                % outcome generated by model -o{m}(g,t)
                %----------------------------------------------------------
                if MDP(m).o(g,t) < 0
                    n             = abs(MDP(m).o(g,t));
                    MDP(m).n(g,t) = n;
    ...
    % get probabilistic outcomes from sampled outcomes or subordinate level
    %======================================================================
    for m = 1:size(MDP,1)
        ...
        for g = 1:Ng(m)
            ...
            % specified as a likelihood or observation (for HMM)
            %--------------------------------------------------------------
            elseif isfield(MDP,'O')
                O{m}{g,t} = MDP(m).O{g}(:,t);
            else
