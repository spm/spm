...
%--------------------------------------------------------------------------
n     = [1/16, 1/16, 1/64, 0]*0;                 %%%% state noise
for f = 1:numel(n)
    b           = GB{f};
    c           = n(f);
...
function O = A_depth(P,f)
% likelihood of depth given some states (s)
% FORMAT O = A_depth(P,f)
%--------------------------------------------------------------------------
global PG
...
if iscell(P)
    ...
    P = logical(P);
    s = size(A);
    O = reshape(A(P,:),[s(2:end),1]);
end

return

% Notes for [O,r]  = A_depth(P,f)
%--------------------------------------------------------------------------
%     r      = find(P,1,'first');
%     r      = min(PG.Nd,r);
%     o      = cell(1,r - 1);
%     q      = sparse(1,1,1,PG.Nc,1);
%     [o{:}] = deal(logical(q));
%     o{end + 1} = logical(1 - q);
%     for d = 1:r
%         o{d}   = o{d}(f{d});
%         siz(d) = numel(f{d});
%     end
%     O  = reshape(spm_cross(o{:}),siz);
%     r  = 1:r;

function O = A_state(P,f)
% likelihood of depth given some states (s)
% FORMAT O = A_state(P,f)
% s  - class or state along ray
%--------------------------------------------------------------------------
global PG
