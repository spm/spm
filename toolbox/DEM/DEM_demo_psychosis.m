...
%--------------------------------------------------------------------------
M.h  = @(x,P)spm_phi([x(:,1) - P.u(1), x(:,2) - P.u(2)]*128);
...
% be estimated
%--------------------------------------------------------------------------
pC   = diag([1 1 1  0 0 0 0  1/64  0 0 0  0 0 0 1/32 1/32 1]);
...
%--------------------------------------------------------------------------
P      = pE;                       % start with prior expectations 
P.A(2) = 21;                       % specify a Rayleigh parameter
P.v    = randn(size(pE.v))/4;      % specify the coefficients of inputs
...
qY    = [];                            % predicted scores
pA    = 8:2:24;                        % multi-start Rayleigh  parameters
for j = 1:n
    
    % generate symptom scores with this subject's Rayleigh parameter
    %----------------------------------------------------------------------
    P      = pE;                       % start with prior expectations
    P.A(2) = rand*16 + 8;              % specify a random subject effect
    P.v    = randn(size(pE.v))/4;      % and some random fluctuations
