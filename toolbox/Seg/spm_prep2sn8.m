function [po,pin] = spm_prep2sn8(p)
...
VG          = p.tpm;
VF          = p.image;
[Y1,Y2,Y3]  = create_def(p.Twarp,VF(1),VG(1),p.Affine);
[Y1,Y2,Y3]  =     spm_invdef(Y1,Y2,Y3,VG(1).dim(1:3),eye(4),eye(4));
MT          =     procrustes(Y1,Y2,Y3,VG(1),VF(1).mat);
d           = size(p.Twarp);
[Affine,Tr] = reparameterise(Y1,Y2,Y3,VG(1),VF(1).mat,MT,max(d(1:3)+2,[8 8 8]));
flags       = struct(...
    'ngaus',    p.ngaus,...
    'mg',       p.mg,...
    'mn',       p.mn,...
...
if nargout>=2,
   % Parameterisation for the inverse
    pin = struct(...
        ...
        'VF',     p.tpm(1),...
        'Tr',     p.Twarp,...
        'Affine', p.tpm(1).mat\p.Affine*p.image(1).mat,...
        'flags',  flags);
    % VG = p.image;
    % VF = p.tpm(1);
    % Tr = p.Twarp;
