...
% IP.fmagP          : Cell containing EPI image used to demonstrate 
...
% IP.mask           : Mask the brain for fieldmap creation
%
...
% 
% IP.graphics       : Disable graphics display if 0
%
...
% 
%_______________________________________________________________________
% FieldMap.m               Jesper Andersson and Chloe Hutton 22/08/05 
% 
...
%=======================================================================
   ...
   case 'welcome'
      ...
      PM = figure('IntegerHandle','off',...
	   'Name',sprintf('%s%s','FieldMap Toolbox, ver. 1.1',...
                          spm('GetUser',' (%s)')),...
	   'NumberTitle','off',...
	   'Tag','FieldMap',...
	   'Position',rect,...
...
%=======================================================================
%
% The functions below may be called by the various gui buttons but are
...
%=======================================================================
   ...
   case 'initialise'
      ...
      IP.mflags = struct('template',[],'fwhm',[],'nerode',[],'ndilate',[],'thresh',[]);
      ...
      % Graphics
      if isfield(pm_def, 'SHOW_GRAPHICS')
         IP.graphics = pm_def.SHOW_GRAPHICS;
      end
      
      % Check whether we can in fact open any windows
...
%=======================================================================
   ...
   varargout{1} = FieldMap('Write',V,vol,'sc',V.dim(4),V.descrip);
...
%=======================================================================
   ...
   case 'createfieldmap'
      ...
      if any(any(diff(ip_dim,1,1),1)&[1,1,1,0])
...
%=======================================================================
   ...
   case 'fm2vdm'
	 ...
	 IP.vdm.ijac = -1*pm_diff(IP.vdm.ivdm,2);
         ...
         FieldMap('Write',IP.vdmP,IP.vdm.ivdm,'',IP.vdmP.dim(4),vdm_info);
      else
         FieldMap('Write',IP.vdmP,IP.vdm.vdm,'',IP.vdmP.dim(4),vdm_info);
      end
      varargout{1} = IP.vdm;
      varargout{2} = IP.vdmP;
...
%=======================================================================
   ...
   case 'write'
      ...
      % Write out image
      img=struct('dim', [V.dim(1:3) varargin{5}],...
                         'mat',     V.mat);
...
%=======================================================================
   ...
   if isfield(IP,'vdmP') & ~isempty(IP.vdmP);
      dP = struct('dim',  [IP.vdmP.dim(1:3) 64],...
                  'pinfo',   [1 0]',...
                  'mat',     IP.vdmP.mat,...
...
%=======================================================================
   ...
   case 'unwarpepi'
      ...
      %
      tiny=5e-2;
      msk = reshape(double(xyz2(:,1)>=(1-tiny) & xyz2(:,1)<=(IP.vdmP.dim(1)+tiny) &...
		           xyz2(:,2)>=(1-tiny) & xyz2(:,2)<=(IP.vdmP.dim(2)+tiny) &...
		           xyz2(:,3)>=(1-tiny) & xyz2(:,3)<=(IP.vdmP.dim(3)+tiny)),IP.epiP.dim(1:3));
...
%=======================================================================
   ...
   case 'matchvdm'
      ...
      % 3) Update space of warped image.
      % 4) Apply resulting transformation matrix to voxel shift map.
      ...
      if IP.epifm==1
         ...
         IP.wfmagP = struct('dim',     [IP.epiP.dim(1:3) 64],...
                         'pinfo',   IP.epiP.pinfo,...
                         'mat',     IP.epiP.mat);
...
%=======================================================================
   ...
   if isempty(IP.fmagP)
      ...
      if isempty(IP.pP) & ~isempty(IP.P{1})
         ...
         IP.fmagP=struct('dim',     [IP.P{1}.dim(1:4)],...
                      'pinfo',   IP.P{1}.pinfo,...
                      'mat',     IP.P{1}.mat);
...
%      end
 ...
 %     end
      ...
      disp('Coregistering fieldmap to EPI...');
