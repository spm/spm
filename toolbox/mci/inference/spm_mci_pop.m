...
%
% If there is a second model you must also specify M{2}.subset ie. 
% that subset of parameters also used by model 2. 
%
...
%     disp('Initialisation point(s): User specified');
% else
%     disp('Initialisation point(s): Drawn from prior');
% end
for j=1:J,
    if isfield(mcmc,'init')
        xinit=mcmc.init{j};
    ...
    if model_switch
        ...
        %[P{j}.logq(1),P{j}.loglike(1)] = tempered_post (Ep,M{1},U{1},Y,P{j}.beta);
...
for i=2:Ntot,
    ...
    if m < gprob
        ...
        %Ep1 = spm_unvec(spm_vec(M.pE) + V*theta1,M.pE);
        %Ep2 = spm_unvec(spm_vec(M.pE) + V*theta2,M.pE);
        %[A,Alike] = tempered_post (Ep2,M,U,Y,P{jsel}.beta);
        %[B,Blike] = tempered_post (Ep1,M,U,Y,P{jsel+1}.beta);
        Ep1=theta1;
        Ep2=theta2;
        ...
        if i<nscale*J
            ...
            if P{jsel}.last_update > update_sigma_steps
                ...
                % old style - cumulative accept rates
                %ar=(P{jsel}.accept+1)/(P{jsel}.accept+P{jsel}.reject+1);
                if ar < 0.2
        ...
        % Accept candidate ?
        accept_prob=min(1,exp(cand_like-P{jsel}.logq(end)));
...
for j=1:J,
    ...
    %disp(sprintf('Chain %d: overall acceptance rate=%1.3f',j,P{j}.ar));
    D.ar(j)=P{j}.ar;
