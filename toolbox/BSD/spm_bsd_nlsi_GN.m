function [Ep,Cp,Eh,F,L,dFdp,dFdpp] = spm_bsd_nlsi_GN(M,U,Y)
% Bayesian inversion of nonlinear models - Gauss-Newton/Variational Laplace
% FORMAT [Ep,Cp,Eh,F,L,dFdp,dFdpp] = spm_bsd_nlsi_GN(M,U,Y)
%
% [Dynamic] MIMO models
...
% Karl Friston - Adapted for BSD by Johan Medrano
...
try
    ...
    for fn = fieldnames(M.pE)'
        try 
            spm_vec(M.P.(fn{1})) - spm_vec(M.pE.(fn{1}));
            if ~M.noprint
                fprintf('\nParameter initialisation successful for %s\n', fn{1})
            end
        catch
            M.P.(fn{1}) = M.pE.(fn{1});
        end
...
%     dfdu = kron(eye(nx,nx),Y.X0);
% catch
    dfdu = zeros(ny,0);
end
if isempty(dfdu), dfdu = zeros(ny,0); end
...
try
    ...
    hE  = zeros(nh,1) - log(var(spm_vec(y))) + 4;
end
h       = hE;
...
for k = 1:M.Nmax
    ...
    if exist('Fsi', 'var')
        ...
        %------------------------------------------------------------------
        [fH, H] = IS(Ep, M, U); 
        H = spm_vec(cellfun(@(c)c.noise, H, 'UniformOutput',0)); 
        e  = spm_vec(e);
        f  = spm_vec(f);
        fH = spm_vec(fH);
        ...
        if isreal(spm_vec(y))
            ...
            subplot(2,1,1)
            plot(x, real(spm_vec(y)), 'g'), hold on
            plot(x, real(spm_vec(y))-dfdu*p(iu), 'b'), hold on
            plot(x,real(f + e),'k'), hold on
            plot(x,real(fH), '-.b', 'LineWidth', 2), hold on
            plot(x,real(H), '--r', 'LineWidth', 2), hold off
            xlabel(xLab)
