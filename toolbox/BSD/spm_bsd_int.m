function [y,H] = spm_bsd_int(P,M,U)
...
%     Q = spm_unvec(spm_vec(P)
    ...
    if nargout > 1
        ...
        for j = 1:numel(Hc.modes)
            Hc.modes{j} = source2sensor(Hc.modes{j}, L, M); 
        end
        Hc.noise = source2sensor(Hc.noise, L, M); 
        H{c}   = Hc; 
    else 
        s  = spm_bsd_csd(Q, M); 
    end
    y{c} = source2sensor(s, L, M);     
end
end

function y = source2sensor(s, L, M)
    % if nargin < 3
    %     norm = sum(s, 1); 
    % end  
    % s = s/norm;
