function [spheres,dipoles,L,Lan,flags] = spm_eegfp_Rsph(model,dipoles,flags)

%____________________________________________________________________________
%
% FUNCTION [spheres,dipoles,L,Lan,flags] = cp_sc2sph(model,dipoles,flags)
% or [spheres] = cp_sc2sph(model,[])
%
...
% Scalp & brain vertices in mm + Electrodes location in mm
	...
	[yM,iM] = max(eXYZ(2,:)); [ym,im] = min(eXYZ(2,:));
...
%     if no(2)<0, no = -no; end % Make sure no points upward
    ...
    if flags.figs & flags.use_br
    	spm_eeg_displScEl(model.head(1),model.electrodes)
    else
        spm_eeg_displScEl(model.head(end),model.electrodes)
    end
...
if flags.calc_dip
	...
	% Display cort surface with colour coded scalp radius
    	fv_c.vertices = dipoles.vert'; fv_c.faces = dipoles.tri'; displ_Cmesh(fv_c,r_ds)
    end
	...
	% Display deformed cort surface
    	fv_Sc.vertices = dipS.vert'; fv_Sc.faces = dipoles.tri'; displ_Cmesh(fv_Sc,(R./r_ds),1);
    end
...
if flags.calc_L
	...
	[Lan,nit] = spm_eegfp_Lana(SdXYZ,SseXYZ,Rsc,Rsk,Rbr,model.sigma);
