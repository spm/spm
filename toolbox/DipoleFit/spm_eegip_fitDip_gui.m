function [sdip,fit_opt,Psave] = spm_eegip_fitDip_gui

% FORMAT [sdip,set_loc_o,Psave] = spm_eegip_fitDip_gui
%
% 'spm_eegip_fitDip_gui' provides the GUI for the dipole fitting routine 'spm_eegip_fitDipS'
% and 'spm_eegip_fitDip'. The former uses a "Realistic sphere forward solution", the later 
% a BEM solution.
%
...
elseif q_data==2
    ...
    error('Something went wrong when selecting the data format...');
end
...
if q_model==1
    if q_data==1
        [sdip,fit_opt] = ...
            spm_eegip_fitDip(squeeze(D.data(order_dat2mod,wind_be(1):wind_be(2),cond(1))), ...
            model,Vbr,n_dip,n_seeds,fit_opt);
    elseif q_data==2
        [sdip,fit_opt] = spm_eegip_fitDip(data(:,wind_be(1):wind_be(2)), ...
            model,Vbr,n_dip,n_seeds,fit_opt);
    end  
elseif q_model==2
    if q_data==1
