function varargout = spm_eegip_DrawDip(action,varargin)
...
% - spm_eegip_DrawDip('Init')
%        The routine asks for the dipoles file and image to display
% - spm_eegip_DrawDip('Init',sdip)
%        The routine asks for the image to display the dipoles on
% - spm_eegip_DrawDip('Init',sdip,P)
%        The routines dispays the dipoles on image P.
%
% If multiple seeds have been used, you can select the seeds to display 
% by pressing their index. If the selected seeds converged to 
% approximately the same location, then all the sources are displayed,
% other wise only the first selected and its selected close neighbours 
% are shown.
% If more than 1 dipole was fitted, then selection of source 1 to N is
% possible through the pull-down selector.
%
% The location of the source is displayed in mm and voxel, as well as
...
% Christophe Phillips,
% $Id: spm_eegip_DrawDip.m 148 2005-05-12 09:42:57Z christophe $
...
case 'init'
%------------------------------------------------------------------------
% FORMAT spm_eegip_DrawDip('Init',sdip,P)
% Initialise the variables with GUI
% e.g. spm_eegip_DrawDip('Init')
%------------------------------------------------------------------------
spm('Clear')
...
sdip.hdl.hcl = uicontrol(Fig,'Style','pushbutton','Position',[310 320 100 20].*WS, ...
        'String','Seeds displayed','CallBack','spm_eegip_DrawDip(''ClearAll'')');
...
for ii=1:sdip.n_seeds
    if sdip.exitflag(ii)==1
        sdip.hdl.hseed(ii) = uicontrol(Fig,'Style','togglebutton','String',num2str(ii),...
            ...
            'CallBack','spm_eegip_DrawDip(''ChgSeed'')');
    else
...
sdip.hdl.hdip = uicontrol(Fig,'Style','popup','String',txt_box, ...
        ...
        'Callback','spm_eegip_DrawDip(''ChgDip'')');
...
% First plot = first seed with convergence !
l_conv = find(sdip.exitflag==1);
if isempty(l_conv)
    error('No seed converged towards a stable solution, nothing to be displayed !')
else
	spm_eegip_DrawDip('DrawDip',l_conv(1),1)
	set(sdip.hdl.hseed(l_conv(1)),'Value',1); % toggle first button
end
...
case 'drawdip'
%------------------------------------------------------------------------
% FORMAT spm_eegip_DrawDip('DrawDip',i_seed,i_dip,sdip)
% e.g. spm_eegip_DrawDip('DrawDip',1,1,sdip)
% e.g. spm_eegip_DrawDip('DrawDip',[1:5],1,sdip)
...
if any(i_seed>sdip.n_seeds) | i_dip>sdip.n_dip
    error('Wrong i_seed or i_dip index in spm_eegip_DrawDip');
end
...
% loc_vx = spm_EEGmodel('mm2vx',sdip.loc{i_seed(1)}(:,i_dip),sdip.M);
...
% Run throught the 6 colors and 9 markers to differentiate the dipoles.
...
%     Mtr = spm_matrix([zeros(1,6) -1*defaults.analyze.flip])*sdip.M;
% Mtr=sdip.M;
%     loc_vx = spm_EEGmodel('mm2vx',sdip.loc{tabl_seed_dip(ii,1)}(:,tabl_seed_dip(ii,2)),Mtr);

    loc_pl = sdip.loc{tabl_seed_dip(ii,1)}(:,tabl_seed_dip(ii,2));

    js = sdip.j{tabl_seed_dip(ii,1)}(tabl_seed_dip(ii,2)*3+l3,sdip.Mtb);
    dip_h(:,ii) = add1dip(loc_pl,js/Mn_j*10,marker(im),colors(ic),st.vols{1}.ax,Fig,st.bb);
    js_m = js_m+js;
end
...
set(sdip.hdl.hor1,'String',[num2str(Njs_m(1)),' ',num2str(Njs_m(2)),' ',num2str(Njs_m(3))]);
...
elseif length(prev_seeds)>length(l_seed)
    ...
    spm_eegip_DrawDip('ClearDip',prev_seeds);
...
spm_eegip_DrawDip('ClearDip');
if ~isempty(i_seed)
    spm_eegip_DrawDip('DrawDip',i_seed,i_dip);
end
...
case 'cleardip'
%------------------------------------------------------------------------
% FORMAT spm_eegip_DrawDip('ClearDip',seed_i)
% e.g. spm_eegip_DrawDip('ClearDip')
%       clears all displayed dipoles
% e.g. spm_eegip_DrawDip('ClearDip',1)
