function varargout = spm_eegfp_model(action,varargin)

% SPM_EEGFP_MODEL is a multi-purpose routine that deals with the generation
% of the head model for the solution of the forward problem.
%
% Called without arguments :
%
% >> model = spm_eegfp_model
% 
...
% FORMAT [Pvol,M,flags] = spm_eegfp_model('GenBin',Pvol,flags);
%
...
% Output :
%   Pvol    : file name of the bin images gnerated.
%   M       : affine transform mapping from tempalte space into image space
%   flags   : all the flags
%__________________________________________________________________________
%
% FORMAT [head,flags] = spm_eegfp_model('GenMesh',Pvol,flags);
%
...
%__________________________________________________________________________
%
% FORMAT [electrodes,flags] = spm_eegfp_model('Elec2Scalp',surf,el_loc,el_name,flags);
%
...
%__________________________________________________________________________
%
% FORMAT [pt_mm] = spm_eegfp_model('vx2mm',pt_vx,M);
%
...
%__________________________________________________________________________
%
% FORMAT [pt_vx] = spm_eegfp_model('mm2vx',pt_mm,M);
%
...
%__________________________________________________________________________
%
% FORMAT [Centre,c_mm] = spm_eegfp_model('CtrBin',P);
%
% Determine the "centre of mass" (vx and mm) of a bin image.
%__________________________________________________________________________
%
% FORMAT [vn,vn_i,S,S_i] = spm_eegfp_model('NormTri',l_tr,ts)
%
...
%__________________________________________________________________________
%
% FORMAT ts = spm_eegfp_model('TesBin',n,Centre,P,info);
%
...
%__________________________________________________________________________
%
% FORMAT ts = spm_eegfp_model('ElastM',ts);
%
...
%__________________________________________________________________________
%
% FORMAT ts = spm_eegfp_model('MeasEd',ts);
%
...
%__________________________________________________________________________
%
% FORMAT ts = spm_eegfp_model('Tr4thPt',ts);
%
...
%__________________________________________________________________________
%
% FORMAT tsph = spm_eegfp_model('TesSph',n,r);
%
...
%__________________________________________________________________________
%
% FORMAT [Pout/val] = spm_eegfp_model('ErodeGrow',P/val,ne,ng,thr_im)
% 
...
%------------------------------------------------------------------------
    ...
    % Project electrodes coord (spher or real) on scalp/brain surface
        ...
        if q_StdElec
            % Select standard electrode set, as defined in 'spm_eegfp_electrset'
            [set_Nel,set_name] = spm_eegfp_electrset;
			el_set = spm_input('Which set of electrodes', ...
					'+1','m',set_name);
     		[el_sphc,el_name] = spm_eegfp_electrset(el_set) ;
    ...
    % Tessalate predefined binarized volume
        ...
        model.param = struct('br_only',0,'Nvol',1,'Mtempl',[],'sigma',[.33 .004 .33]);
...
%         if gener==3, load(Pmod); end
        [electrodes,flags_el] = spm_eegfp_model('Elec2Scalp',model.head(end), ...
    ...
    % Define realistic sphere model
        ...
        if isfield(model,'br_only')
            flags_Rs.br_only = model.br_only;
        else
            flags_Rs.br_only = 0;
        end
        [spheres,a,b,c,flags_Rs] = spm_eegfp_Rsph(model,[],flags_Rs);
...
case 'genbin'
%------------------------------------------------------------------------
% FORMAT [Pvol,M,flags] = spm_eegfp_model('GenBin',Pvol,flag_bi);
%
% Generate the binarised image of the brain (and scalp if the image allows).
% 
% Input :
...
% Output :
%   Pvol    : file name of the bin images gnerated.
%   M       : affine transform mapping from template space into image space
%------------------------------------------------------------------------
    fprintf('\n\n'), fprintf('%c','='*ones(1,80)), fprintf('\n')
    fprintf(['\tGenerate binarized volumes from image.\n']);
    Pvol = varargin{1} ;
    def_flags = struct('img_type',1,'img_norm',0,'ne',1,'ng',3,'thr_im',[.9 .1]);
    if nargin<3
        flags = def_flags;
    else
        flags = varargin{2};
...
% 1.Segment image, generate the brain mask and estimate 
%   the mapping between template and image space.
    if flags.img_norm
        Ptempl = eye(4);
	else
...
%   and write the *_obrain img on disk
    Vbr = VO(4);
	...
	for pp=1:Vbr.dim(3),
		Vbr = spm_write_plane(Vbr,double(Vbr.dat(:,:,pp)),pp);
    ...
    if flags.img_type==1
% 3.Generate the outer-scalp volume, if possible
		Vsc = spm_vol(Pvol);
		Vsc.dat = loaduint8(Vsc);
        
		ne = flags.ne(end); ng = flags.ng(end); thr_im = flags.thr_im(end);
		Vsc.dat = spm_eegfp_model('ErodeGrow',Vsc.dat,ne,ng,thr_im);
		
        % The bottom part of the image needs to be masked out.
        ...
        % Write the file
        Vsc.fname = fullfile(pth,[nm,'_oscalp',xt]);
    ...
    Mtempl = M;
    save(fullfile(pth,[nm,'_Mtempl.mat']),'Mtempl');
    varargout{2} = M;
    varargout{3} = flags;
...
%------------------------------------------------------------------------
	...
	c_mm = c_mm/SI;
    Centre = spm_eegfp_model('mm2vx',c_mm,Vp.mat)';
    varargout{1} = Centre;
    varargout{2} = c_mm;
    
% Old version where I check the min and max voxel index in each direction.
% It's easy but can easily get biased !
% Plus it's not elegant...
% 	slice = zeros(1,Vp.dim(3)) ;
% 	xmin = Vp.dim(1) ;
% 	xmax = -1 ;
% 	ymin = Vp.dim(2) ;
% 	ymax = -1 ;
...
% 	for j = 1:Vp.dim(3)
% 		B = spm_matrix([0 0 j ]);
% 		dp  = spm_slice_vol(Vp,B,Vp.dim(1:2),1)/Vp.pinfo(1) ;
% 		dp_tmp = dp(:) ;
% 		if any(dp_tmp~=0)
% 			slice(j) = 1 ;
% 		end
% 		if any(max(dp)~=0)
% 			ymin_tmp = min(find(max(dp)~=0)) ;
% 			ymin = min([ymin ymin_tmp]) ;
% 			ymax_tmp = max(find(max(dp)~=0)) ;
% 			ymax = max([ymax ymax_tmp]) ;
% 		end
% 		if any(max(dp')~=0)
% 			xmin_tmp = min(find(max(dp')~=0)) ;
% 			xmin = min([xmin xmin_tmp]) ;
% 			xmax_tmp = max(find(max(dp')~=0)) ;
% 			xmax = max([xmax xmax_tmp]) ;
% 		end
% 	end
% 	zmin = min(find(slice)) ; zmax  = max(find(slice)) ;
% 	% Centre = [xmin+xmax ymin+ymax zmin+zmax] / 2 ;
% 	% minMax = struct('xm',xmin,'xM',xmax,'ym',ymin,'yM',ymax,'zm',zmin,'zM',zmax)
% 	varargout{1} = [xmin+xmax ymin+ymax zmin+zmax] / 2 ;
% 	varargout{2} = struct('xm',xmin,'xM',xmax,'ym',ymin,'yM',ymax,'zm',zmin,'zM',zmax) ;
% 	varargout{3} = slice ;

%________________________________________________________________________
case 'elec2scalp'
%------------------------------------------------------------------------
% FORMAT electrodes = spm_eegfp_model('Elec2Scalp',surf,el_loc,el_name,flags);
%
% Projecting the electrodes (in spherical or realistic coordinates) 
% on the realistic head model.
% 
...
%------------------------------------------------------------------------
    ...
    if nargin<4
        [set_Nel,set_name] = spm_eegfp_electrset;
        try 
            [kk,el_name] = spm_eegfp_electrset(find(set_Nel==Nel));
        catch
            el_name = [];
        end
    else
...
case 'genmesh'
%------------------------------------------------------------------------
% FORMAT head = spm_eegfp_model('GenMesh',Pvol,flags);
%
% Generate the tesselated surface of the 1 to 3 head volumes
% (brain-skull-scalp) from the binarized volumes.
% Input :
...
%------------------------------------------------------------------------
    ...
    [Centre_vx,Centre_mm]=spm_eegfp_model('CtrBin',Pvol(1,:));
    Nbin = size(Pvol,1);
    ...
    mesh_labels = strvcat('Tess. scalp','Tess. skull','Tess. brain');
    if flags.br_only
        head = spm_eegfp_model('TesBin',n(1),Centre_mm,Pvol,mesh_labels(3,:));
    elseif Nbin==2 % Brain & scalp only, no skull.
        head(1) = spm_eegfp_model('TesBin',n(1),Centre_mm,Pvol(1,:),mesh_labels(3,:));    
        head(2) = spm_eegfp_model('TesBin',n(2),Centre_mm,Pvol(2,:),mesh_labels(1,:));    
    else
        for ii=Nbin:-1:1
            head(ii) = spm_eegfp_model('TesBin',n(ii),Centre_mm,Pvol(ii,:),mesh_labels(ii,:));
        end
    end
    if flags.q_elast_m
    	fprintf(['\t\tCorrect position of vertices\n']);
        for ii=Nbin:-1:1
            head(ii) = spm_eegfp_model('ElastM',head(ii),Pvol(ii,:));
        end
    end
    if flags.q_meased
        ...
        for ii=Nbin:-1:1
            head(ii) = spm_eegfp_model('MeasEd',head(ii));
        end
    end
    if flags.q_4thpt
        ...
        head(1).pt4 = [];
        for ii=Nbin:-1:1
            head(ii) = spm_eegfp_model('Tr4thPt',head(ii),Pvol(ii,:));
        end
    end
    varargout{1} = head;
...
case 'tesbin'
%------------------------------------------------------------------------
% FORMAT ts = spm_eegfp_model('TesBin',n,ctr_vol,P,info);
%
% Generate a mesh covering a binarized volume
%   1. Generate a simple spherical mesh
%   2. The spherical mesh is projected radially on the bin volume
...
%------------------------------------------------------------------------
	...
	%---------------------------
	tsph = spm_eegfp_model('TesSph',n,rad);             % tesselated sphere of radius rad,
	...
	for i = 1:tsph.nr(1)
        ...
        line_vx = spm_eegfp_model('mm2vx',line,Vv.mat);
	...
	srf_vert_mm = spm_eegfp_model('vx2mm',srf_vert,Vv.mat);
...
% %             Ptempl = eye(4);
% %         end
% %         spm_segment(Pvol,Ptempl);
% %         Pseg_i = [];
% %         for ii=1:2
% %             Pseg_i = strvcat(Pseg_i, ...
% %                [spm_str_manip(Pvol,'r'),'_seg',num2str(ii),'.img']);
% %         end
% %         Vseg_i = spm_vol(Pseg_i);
% %         Vsum = Vseg_i(1);
% %         Vsum.fname = [spm_str_manip(Pvol,'r'),'_obrain.img'];
% %         Vsum.descrip = 'sum of 3 seg. images -> outer brain volume';
% %         Vsum = spm_create_vol(Vsum);
% %         Vsum.pinfo(1,1) = spm_add(Vseg_i,Vsum)   
% %         Vsum = spm_close_vol(Vsum);
% %         Vsum = spm_create_vol(Vsum);
% %         Vsum = spm_close_vol(Vsum);
% %         % It needs some erosion-growing !
% %         ne = 2; ng = 4; thr_im = .9;
% %         [Pvol_bin] = spm_eegfp_model('ErodeGrow',[Vsum.fname;Vsum.fname],ne,ng,thr_im);
% % 
% %         % If dealing with sMRI, create scalp volume
% %         if img_type==1
% %             ne = 2; ng = 2; thr_im = .1;
% %             Psc = [spm_str_manip(Pvol,'r'),'_oscalp.img']
% %             Psc = spm_eegfp_model('ErodeGrow',strvcat(Pvol,Psc),ne,ng,thr_im);
% %             Pvol_bin = strvcat(Pvol,Psc);
% %         end
% %         % Mesh it
% %         Pvol = Pvol_bin;
