...
% John Ashburner
% $Id: spm_shoot3d.m 4675 2012-03-02 19:49:35Z john $
...
[ phi, Jphi]     = shoot3('smalldef', vt,1/N);
...
if nargout>=5,
    [theta,Jtheta] = shoot3('smalldef', vt,-1/N);
elseif nargout>=4,
    theta          = shoot3('smalldef', vt,-1/N);
end

for t=2:abs(N),
    mt             = shoot3('pushg',m0,phi,Jphi);
    ...
    [ phi, Jphi]   = shoot3('comp', dp, phi, dJ, Jphi); % Build up large def. from small defs
...
%imagesc(dj); axis image; drawnow
    ...
    if nargout>=5,
        [   dp,    dJ] = shoot3('smalldef',  vt,-1/N);        % Small deformation
        [theta,Jtheta] = shoot3('comp', theta,dp, Jtheta,dJ); % Build up large def. from small defs
        clear dp dJ
    elseif nargout>=4,
        dp             = shoot3('smalldef',  vt,-1/N);        % Small deformation
        theta          = shoot3('comp', theta, dp);           % Build up large def. from small defs
        clear dp
    end
    drawnow
end
