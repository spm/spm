...
opts0.priors   = str2mat(...
               fullfile(dir,'grey.img'),...
               fullfile(dir,'white.img'),...
               fullfile(dir,'csf.img'));
opts0.ngaus    = [3 2 2 5];
opts0.warpreg  = 1;
opts0.warpco   = 25;
opts0.biasreg  = 0.00001;
opts0.biasfwhm = 75;
opts0.regtype  = 'mni';
opts0.fudge    = 5;
opts0.samp     = 3;
...
if 1,
    ...
    for subit=1:40,
        ...
        if subit > 1 && (ll+llrb)<(oll+ollrb),
            ...
            spm_chi2_plot('Set',ll+llr+llrb);
            oTbias = Tbias;
            if subit > 1 && (ll+llrb-oll-ollrb)<tol1*nm,
...
if 1,
    ...
    for z=1:length(z0),
        ...
        for k1=1:Kb,
            ...
            pp = zeros(buf(z).nm,1);
            for k=kk,
                pp = pp + exp((cr-mn(k)).^2/(-2*vr(k)))/sqrt(2*pi*vr(k)+tiny)*mg(k);
            end;
            pp      = pp.*bf;
            q(:,k1) = pp;
