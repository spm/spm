...
%
% The parameters returned by Bayesian Model Averaging (BMA) are the 'latent' variables
% A and B which are Gaussian (and consequently can be positive or negative).  
%
...
% about.
%
% This routine computes the posterior distribution over cA or gB by
% generating samples from the latent variables, and exponentiating each
% sample. 
%
% The probability that the rA or RB values are greater than some threshold xt
% (such as unity) is then just the proportion of posterior samples that
% are greater than xt.
%
% If a contrast matrix (C) is not specifed this function looks at a single connection
% or gain. To look at relative sizes connection/gain values enter a .C
% matrix. eg. to test, in a 3 region DCM, is connection from 3 to 2 bigger than 2 to 3 ? 
% set C=[0 0 0; 0 0 1; 0 -1 0]
% 
% Example usage: 
%
...
% Will Penny
% $Id: spm_dcm_erp_bma.m 4528 2011-10-18 14:37:26Z will $

try
    load (BMS_file);
catch
    BMS_file = spm_select(1,'mat','Select BMS.mat file',[],[],'BMS.mat');
    load (BMS_file);
end
...
switch lower(stats)
    ...
    case 'rfx',
        bma=BMS.DCM.rfx.bma;
    otherwise
        disp('The stats field shout be set to ''ffx'' or ''rfx''');
        return
end
...
switch params.type
    ...
    otherwise
        disp('conn_type must be ''A'' or ''B''');
        return
end
        

% Generate N samples in latent space, then exponentiate to 
% get estimate of effects
N=10000;
...
disp(str);
disp(sprintf('Posterior mean = %1.2f',rm));
disp(sprintf('Exceedance probability = %1.6f', xp));
