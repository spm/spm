...
% Karl Friston
% $Id: spm_dcm_csd.m 4095 2010-10-22 19:37:51Z karl $
...
% adjust gain to accomodate scaling differences among models
%--------------------------------------------------------------------------
Y         = feval(DCM.M.IS,DCM.M.pE,DCM.M,DCM.xU);
DCM.M.U   = U*sqrt(norm(abs(spm_vec(DCM.xY.y)),'inf')/norm(abs(spm_vec(Y,'inf'))));

% adjust priors
%--------------------------------------------------------------------------
DCM.M.P   = spm_dcm_csd_priors(DCM.M,DCM.xU,DCM.xY);
