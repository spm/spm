...
method.help    = {'Specify inference method: random effects (2nd-level)'...
                  'or fixed effects (1st-level) analysis.'};
method.labels  = {
                  'Fixed effects'
                  'Random effects'
}';
...
file.tag     = 'file';
file.name    = 'BMS.mat';
file.help    = {'Specify the BMS (.mat) file obtained from BMS Maps'...
               '(Inference). Leave field empty to work on serial mode.'};
file.filter  = 'mat';
file.ufilter = '.*';
file.val     = {{''}};
file.num     = [0 1];
...
bms.values  = {bms_dcm bms_map_inf bms_map_vis };
