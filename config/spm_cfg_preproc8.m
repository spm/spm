...
%--------------------------------------------------------------------------
% write Save INU corrected
%--------------------------------------------------------------------------
write         = cfg_menu;
write.tag     = 'write';
write.name    = 'Save INU corrected';
write.help    = {
...
data.name    = 'Data';
data.val     = {channel };
data.help    = {['Specify the number of different channels (for multi-spectral '...
    'classification). If you have scans of different contrasts for each of the '...
    'subjects, then it is possible to combine the information from them in order '...
    'to improve the segmentation accuracy. Note that only the first channel of '...
    'data is used for the initial affine registration with the tissue probability maps.'],...
    ['Note that floating point images can cause problems. '...
    'If using floating point images, then make sure missing data is assigned '...
    'values of NaN (rather than 0), and that the intensity distribution '...
    'is relatively continuous, rather than a set of delta functions at (e.g.) integer values.']}';
data.values  = {channel };
data.num     = [1 Inf];
