...
template.val = {{fullfile(spm('Dir'),'templates','T1.mnc')}};
...
affreg.help = spm_justify(w,...
'An affine registration needs to be done for non spatially normalised images.');
...
unnecessary.help = spm_justify(w,...
...
%affreg1.values = {...
%fullfile(spm('Dir'),'templates','T1.mnc'),...
...
overlay.help = spm_justify(w,...
'How should the priors be overlayed over the image to segment.');
...
wrt_cor.help   = spm_justify(w,'Write bias correted versions of the images to disk');
...
if isfield(job.data,'singlespec'),
    ...
    for i=1:numel(job.data.singlespec),
        ...
        sub = {...
            fullfile(pth,[nam '_seg1.img' num]),...
            fullfile(pth,[nam '_seg2.img' num]),...
            fullfile(pth,[nam '_seg3.img' num]),...
            fullfile(pth,['m' nam '.img' num])};
        for j=1:n,
            vf{j,i} = sub{j};
        end;
    end;
