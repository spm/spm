...
uicontrol(Fmenu,'Style','Text','String','Causal modeling',...
...
%-Spatial
%-----------------------------------------------------------------------
uicontrol(Fmenu,'String','Realign',	'Position',[040 380 080 030].*WS,...
...
uicontrol(Fmenu,'String','Smooth',	'Position',[280 380 080 030].*WS,...
	'ToolTipString','spatial smoothing with Gaussian kernel',...
	'CallBack','spm_smooth_ui;');

uicontrol(Fmenu,'String','Coregister',	'Position',[040 345 080 030].*WS,...
...
%-Statistical
%-----------------------------------------------------------------------
uicontrol(Fmenu,'String','Basic models','Position',[035 285 110 030].*WS,...
	'ToolTipString','basic stats  models for independent data',...
	'CallBack','SPM = spm_spm_ui(''cfg'',spm_spm_ui(''DesDefs_Stats''));')

uicontrol(Fmenu,'String','PET/SPECT','Position',[150 285 95 030].*WS,...
...
uicontrol(Fmenu,'String','fMRI','Position',[150 285 95 030].*WS,...
	'ToolTipString',['general linear model setup for serially ',...
		'correlated fMRI time series'],...
	'CallBack','SPM = spm_fmri_spm_ui;',...
	'Visible','off','Tag','FMRI')

uicontrol(Fmenu,'String','Review design','Position',[250 285 110 030].*WS,...
	'ToolTipString','review a previously specified model',...
	'CallBack','spm pointer watch, spm_DesRep; spm pointer arrow')

str     = ['load(spm_get(1,''SPM.mat'',''Select SPM.mat...''));spm_spm(SPM);'];

uicontrol(Fmenu,'String','Estimate {OLS}','Position',[035 250 160 030].*WS,...
	'ToolTipString','estimate the parameters of a specified model',...
	'CallBack',str)

uicontrol(Fmenu,'String','-> Bayesian','Position',[200 250 160 030].*WS,...
	'ToolTipString','Bayesian or conditional estimators',...
	'CallBack','spm_spm_Bayes',...
	'ForeGroundColor',[0 0 1])

uicontrol(Fmenu,'String','Results {SPM or PPM}',...
	'Position',[150 205 210 030].*WS,...
	'ToolTipString','Inference and regional responses etc.',...
	'CallBack','[hReg,xSPM,SPM] = spm_results_ui;')

uicontrol(Fmenu,'String','Specify DCM','Position',[150 160 115 030].*WS,...
	'ToolTipString','Specify and estimate a dynamic causal model',...
	'CallBack','spm_dcm_ui(''specify'')',...
	'Visible','off','Tag','FMRI',...
	'ForeGroundColor',[0 0 1])

uicontrol(Fmenu,'String','Results','Position',[275 160 85 030].*WS,...
	'ToolTipString','review the results of a DCM',...
	'CallBack','spm_dcm_ui(''review'')',...
	'ForeGroundColor',[0 0 1])
...
uicontrol(Fmenu,...
	...
	'FontSize',FS(9),		'CallBack','spm_peb_ppi')
...
uicontrol(Fmenu,'String','Bias cor',	'Position',[298 054 082 024].*WS,...
	'ToolTipString','Bias correction',...
	'FontSize',FS(9),		'CallBack','spm_bias_ui;')
