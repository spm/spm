...
% Andrew Holmes
% $Id: spm.m 781 2007-04-02 22:08:29Z klaas $
...
uicontrol(F,'String','PET and SPECT',...
	...
	'Position',[140 061 115 030],...
	'CallBack','delete(gcbf),clear all,spm(''PET'')',...
	'ForegroundColor',[0 1 1])

uicontrol(F,'String','M/EEG',...
	'ToolTipString',...
	'launch SPM-GUI in EEG/MEG modality (or type "spm eeg" in MatLab)',...
	'Position',[265 061 70 030],...
	'CallBack','delete(gcbf),clear all,spm(''EEG'')',...
	'ForegroundColor',[0 1 1])

uicontrol(F,'String','fMRI time-series',...
	'ToolTipString',...
	'launch SPM-GUI in fMRI modality (or type "spm fmri" in MatLab)',...
	'Position',[345 061 115 030],...
...
uicontrol(Fmenu,'Style','Text',...
  'String','temporal pre-processing',...
	'Position',[025 410 350 020].*WS,...
	'ForegroundColor','w',...
    'FontAngle','Italic',...
	'FontSize',FS(10),...
...
uicontrol(Fmenu,'Style','Text',...
	'String','Model specification, review and estimation',...
...
uicontrol(Fmenu,'String','Realign',...
	...
	'FontSize',FS(10),...
	'ToolTipString','realignment',...
	'UserData','jobs.spatial.realign',...
	'CallBack','spm_jobman(''interactive'','''',''jobs.spatial.realign'');',...
	'Tag','PET','Visible','off');
...
uicontrol(Fmenu,'String','Normalize',...
	...
	'CallBack','spm_jobman(''interactive'','''',''jobs.spatial.normalise'');',...
	'Tag','PET',...
	'UserData','jobs.spatial.normalise',...
	'Visible','off');
...
uicontrol(Fmenu,'String','Slice timing',...
	...
	'CallBack','spm_jobman(''interactive'','''',''jobs.temporal.st'');',...
	'UserData','jobs.temporal.st',...
	'Tag','FMRI',...
	'Visible','off');;
...
uicontrol(Fmenu,'String','Smooth',...
	...
	'FontSize',FS(10),...
	'UserData','jobs.spatial.smooth',...
	'CallBack','spm_jobman(''interactive'','''',''jobs.spatial.smooth'');',...
    'Tag', 'PETFMRI');
...
uicontrol(Fmenu,'String','Coregister',...
	...
	'FontSize',FS(10),...
	'UserData','jobs.spatial.coreg',...
	'CallBack','spm_jobman(''interactive'','''',''jobs.spatial.coreg'');',...
    'Tag', 'PETFMRI');
...
uicontrol(Fmenu,'String','Segment',...
	...
	'FontSize',FS(10),...
	'UserData','jobs.spatial.preproc',...
	'CallBack','spm_jobman(''interactive'','''',''jobs.spatial.preproc'');',...
    'Tag', 'PETFMRI');
...
uicontrol(Fmenu,'String','Convert',...
    'Position',[035 380 100 030].*WS,...
	'ToolTipString','Convert neuroscan format',...
	'FontSize',FS(10),...
    'UserData','spm_eeg_converteeg2mat',...
	'CallBack','spm_eeg_converteeg2mat;',...
    'Tag', 'EEG',...
    'Visible','off')

uicontrol(Fmenu,'String','Epoching',...
    'Position',[035 345 100 030].*WS,...
	'ToolTipString','epoch continuous M/EEG data',...
	'FontSize',FS(10),...
    'UserData','spm_eeg_epochs',...
	'CallBack','spm_eeg_epochs;',...
    'Tag', 'EEG',...
    'Visible','off');
	
uicontrol(Fmenu,'String','Filter',...
    'Position',[150 380 100 030].*WS,...
	'ToolTipString','rereference M/EEG time-series',...
	'FontSize',FS(10),...
    'UserData','spm_eeg_filter',...
    'CallBack','spm_eeg_filter;',...
    'Tag', 'EEG',...
    'Visible','off');
...
uicontrol(Fmenu,'Style','PopUp',...
    'String','Other...|rereference|downsample|time-frequency|grand mean|merge|3Dimages|average TF|contrast',...
	'Position',[265 345 100 030].*WS,...
	'ToolTipString','Other preprocessing functions',...
	'FontSize',FS(10),...
    'CallBack','spm(''PopUpCB'',gcbo)',...
	'UserData',{'spm_eeg_rereference;','spm_eeg_downsample;','spm_eeg_tf;',...
    'spm_eeg_grandmean;','spm_eeg_merge;','spm_eeg_make3dimage;','spm_eeg_average_TF;','spm_eeg_weight_epochs;'},...
    'Tag', 'EEG')
...
uicontrol(Fmenu,'String','3D source reconstruction',...
...
uicontrol(Fmenu,'String','Specify 1st-level',...
	'Position',[75 285 110 030].*WS,...
	'ToolTipString',['GLM setup for fMRI time series'],...
	'FontSize',FS(10),...
	'CallBack','spm_jobman(''interactive'','''',''jobs.stats.fmri_spec'');',...
	'Visible','off',...
	'UserData','jobs.stats.fmri_spec',...
	'Tag','FMRI');

uicontrol(Fmenu,'String','Specify 2nd-level',...
	'Position',[75 250 110 030].*WS,...
	'FontSize',FS(10),...
	'ToolTipString','basic statistical models',...
	'UserData','jobs.stats.factorial_design',...
	'CallBack','spm_jobman(''interactive'','''',''jobs.stats.factorial_design'');',...
    'Tag', 'FMRI');

uicontrol(Fmenu,'String','Review',...
	'Position',[225 285 110 030].*WS,...
	'FontSize',FS(10),...
...
uicontrol(Fmenu,'String','Estimate',...
	'Position',[225 250 110 030].*WS,...
	'ToolTipString','estimate parameters of a specified model',...
	'FontSize',FS(10),...
	'UserData','jobs.stats.fmri_est',...
	'CallBack','spm_jobman(''interactive'','''',''jobs.stats.fmri_est'');',...
    'Tag', 'FMRI');
...
uicontrol(Fmenu,'String','Estimate',...
	...
	'ToolTipString','estimate (ReML) parameters of a specified model',...
	'FontSize',FS(10),...
	'UserData','spm_spm',...
...
uicontrol(Fmenu,'String','Specify 1st-level',...
	'Position',[75 240 110 030].*WS,...
	'FontSize',FS(10),...
	'ToolTipString','Compute averages and other weighting functions of M/EEG responses',...
	'UserData','spm_eeg_firstlevel',...
	'CallBack','spm_eeg_firstlevel;',...
    'Tag', 'EEG');

uicontrol(Fmenu,'String','Specify 2nd-level',...
	'Position',[75 205 110 030].*WS,...
	'FontSize',FS(10),...
	'ToolTipString','2nd (or higher) level models for M/EEG responses',...
	'UserData','jobs.stats.factorial_design',...
	'CallBack','spm_jobman(''interactive'','''',''jobs.stats.factorial_design'');',...
    'Tag', 'EEG');
...
uicontrol(Fmenu,'String','Display',...
	...
	'FontSize',FS(9),...
	'UserData','jobs.util.disp',...
	'CallBack','spm_jobman(''serial'','''',''jobs.util.disp'');',...
    'Tag', 'PETFMRI')
...
uicontrol(Fmenu,'Style','PopUp',...
	...
	'FontSize',FS(9),...
    'CallBack','spm(''PopUpCB'',gcbo)',...
	'UserData',{	'spm_jobman(''serial'','''',''jobs.util.disp'');','spm_eeg_display_ui;'},...
    'Tag', 'EEG')
