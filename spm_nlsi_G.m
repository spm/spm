...
for k = 1:4
    ...
    %----------------------------------------------------------------------
    g     = x*G';
    ey    = spm_vec(y) - spm_vec(g) - dgdu*Eu;
    ...
    % errors and conditional covariances
    %----------------------------------------------------------------------
    ep    = Vp'*(spm_vec(Ep) - spm_vec(pE));
    eg    = Vg'*(spm_vec(Eg) - spm_vec(gE));
    eu    =      spm_vec(Eu) - spm_vec(uE);

    dLdpp = dgdp'*iS*dgdp + ipC;
    dLdgg = dgdg'*iS*dgdg + igC;
    dLduu = dgdu'*iS*dgdu + iuC;
    Cp    = inv(dLdpp);
    Cg    = inv(dLdgg);
    Cu    = inv(dLduu);
    ...
    dg    = spm_dx(dFdgg,dFdg,{t});
    du    = spm_dx(dFduu,dFdu,{t});
    ...
    try
        ...
        plot([1:ns]*Y.dt,g + spm_unvec(ey,g),':'), hold off
        xlabel('time')
