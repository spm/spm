...
%
%---------------------------------------------------------------------
% NOTES ON QUALITY OF DECONVOLUTION
% Although the choice of white noise priors works for the parametric 
% empirical Bayes deconvolution works in many instances, it may not 
% work well for all cases. The deconvolution  has problems particularly
% when the TR is greater than 4 seconds. The graphical output of this
% function displays the original BOLD signal and the estimate of that
% signal based on reconvolving the neuronal response with an HRF. That
% graph should always be checked to make sure the BOLD and its estimate
...
% This is given to the user for convenience.
%-------------------------------------------------------------------------
PPI.Y = Y;
...
% compute reconvolved neuronal responses for graphing
%----------------------------------------------------------------------
if ~ppiflag
    for i = 1:2
        reHRF{i} = conv(xn(:,i),hrf);
        reHRF{i} = reHRF{i}(1:N);
    end
else
    reHRF{1} = conv(xn(:,1),hrf);
    reHRF{1} = reHRF{1}(1:N);
end
...
% plot time vs. original and reconstituted physiologic variable
% -------------------------------------------------------------------------
plot(x,Y(:,1)/max(Y(:,1)),'k-',x,reHRF{1}/max(reHRF{1}),'r:')
xlabel('time (secs)')
ylabel('hemodynamic and neuronal responses')
title('Phsyiological variable')
h = legend('BOLD','Reconv. neural',4);
set(h,'fontsize',8)
axis tight square
grid on
...
subplot(2,2,4)
if ~ppiflag
    plot(x,Y(:,2)/max(Y(:,2)),'k-',x,reHRF{2}/max(reHRF{2}),'r:')
    ylabel('hemodynamic and neuronal responses')
