...
% Guillaume Flandin
% $Id: spm_ov_browser.m 8268 2022-06-23 16:36:14Z guillaume $
...
ischeckreg = isempty(findobj(st.fig,'Tag','spm_image:reorient'));
h = 0.025; if ~ischeckreg, h = h/10; end

hS = uicontrol('Parent', Fgraph,...
    'Style',             'slider',...
    'Units',             'normalized',...
    'Position',          [0.1 h 0.8 0.02],...
    'Min',               1,...
    'Max',               numel(f),...
    'Value',             1,...
...
if ischeckreg
    ...
    hT = xlabel(st.vols{hC}.ax{1}.ax,f{1},'Interpreter','none');
end
...
if strcmp(get(hT,'Type'),'text')
    try, xlabel(st.vols{hC}.ax{1}.ax,f{i},'Interpreter','none'); end
else
