function [outfilenames]=spm_eeg_ft_beamformer_mult(S)
% MOdified version of Vladimir's DICs script.
% this computes a t stat and a multivarariate (Hotellings) beamformer image
%% both computations done on power
%(images using Sekihara eigenval approach to chose optimal direction). Requires as input
...
for i=1:length(grid.inside),
    ...
    if i/100==floor(i/100)
        disp(sprintf('done Hotelling t stats for %3.2f percent of freq band %d of %d',100*i/length(grid.inside),f,Nbands));
    end; % if 
end;
