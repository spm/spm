...
%             Vsens(3) relates to an baseline estimate of sensor noise
...
% Jeremie Mattout
% $Id: spm_eeg_inv_inverse.m 621 2006-09-12 17:22:42Z karl $
...
% Anti-average estimate of noise covariance matrix
% currently diasabled because it does not work for contrasts
%--------------------------------------------------------------------------
if Vsens(2)
    D.inv{val}.inverse.priors.level1{2}.status = 'yes';
    if isempty(D.inv{val}.inverse.priors.level1{2}.filename)
        Qe{end + 1} = spm_eeg_inv_AntiAverage(D.fname,D.inv{val}.inverse.contrast,D.inv{val}.inverse.woi);
    else
        variabl     = load(D.inv{val}.inverse.priors.level1{2}.filename);
        name        = fieldnames(variabl);
        Qe{end + 1} = getfield(variabl , name{1});
    end
else
    D.inv{val}.inverse.priors.level1{2}.status = 'no';
end

% Baseline estimate of noise covariance matrix
%--------------------------------------------------------------------------
if Vsens(3)
    D.inv{val}.inverse.priors.level1{3}.status = 'yes';
    if isempty(D.inv{val}.inverse.priors.level1{3}.filename)
        Qe{end + 1} = spm_eeg_inv_Baseline(D.fname,D.inv{val}.inverse.contrast);
    else
        variabl     = load(D.inv{val}.inverse.priors.level1{3}.filename);
        name        = fieldnames(variabl);
        Qe{end + 1} = getfield(variabl , name{1});
    end
else
...
if Vsour(1) % Compute Smoothness Prior
    ...
    if any(size(Mdist) - [Nv Nv])
        errordlg({'Wrong mesh size','Please recomute mesh'});
        return
    end
...
if Vsour(2) % Minimum Norm prior
    Qp{end + 1} = speye(Nv,Nv);
    D.inv{val}.inverse.priors.level2{2}.status = 'yes';
else
    D.inv{val}.inverse.priors.level2{1}.status = 'no';
end

% Mutlivariate Source Prelocalisation (MSP) prior
%--------------------------------------------------------------------------
if Vsour(3) | (Nv - D.inv{val}.inverse.dim)
    ...
    for i = 1:Nprior
        if Vsour(3+i)
            if ~isempty(D.inv{val}.inverse.priors.level2{3+i}.filename)
                ...
                Qp{end + 1} = getfield(variabl , name{1});
                D.inv{val}.inverse.priors.level2{3+i}.status = 'yes';
                if any(size(Qp{end}) - [Nv Nv])
...
for i = 1:length(Inz)
    Trials = find(D.events.code == D.events.types(Inz(i)));
    n(i)   = length(Trials);
    if round(n(i)/2) ~= n(i)/2
        n(i) = n(i) - 1;
        if ~n
            warndlg({'please try again','This file has only one trial'})
            return
        end
    end
    ...
    for k = 1:n(i)
        ...
        Yi = [Yi s*squeeze(D.data(j,woi(1):woi(2),Trials(randvec(k))))];
    end
...
%=======================================================================
function Cbb = spm_eeg_inv_Baseline(fname,contrast)
% Compute an empirical estimate of the noise covariance matrix
% using the prestimulus baseline

try
    D = spm_eeg_ldata(fname);
catch
    D = spm_select(1, '.mat', 'Select data for baslein averaging');
    D = spm_eeg_ldata(D);
end
