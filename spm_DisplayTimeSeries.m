...
%       .pos3: a 4x1 vector containing the position of the tempora slider
...
ud.v.handles.axes = axes('parent',hp,'position',pos1,...
    ...
    'tag',tag,'nextplot','add','drawmode','fast');
ud.v.handles.gpa  = axes('parent',hp,'position',pos2,...
    'tag',tag,'nextplot','add','ytick',[],...
    'box','off','color','none','ygrid','off',...
    'drawmode','fast');
...
ud.v.handles.gpp = plot(ud.v.handles.gpa,ud.v.ind,ud.v.y2,'color',0.5*[1 1 1]);
...
if ~isempty(ud.v.et)
    for i=1:length(ud.v.et)
        ...
        ud.v.et(i).hp = plot(ud.v.handles.axes,ud.v.et(i).time.*[1 1],...
            [ud.v.mi,ud.v.ma],'color',col(ud.v.et(i).col,:));
        set(ud.v.et(i).hp,'userdata',i,...
            'ButtonDownFcn','set(gco,''selected'',''on'')',...
            'Clipping','on');
    end
end

% Add display patch and borders
ud.v.handles.pa = patch('xdata',[itw(1) itw(end) itw(end) itw(1)],'ydata',[mi mi ma ma],...
    'parent',ud.v.handles.gpa,'edgecolor','none','facecolor',[.5 .5 .5],...
    'facealpha',0.5,'ButtonDownFcn',@doPatch,'interruptible','off');
ud.v.handles.lb = plot(ud.v.handles.gpa,[itw(1) itw(1)],[mi ma],'k',...
    'buttondownfcn',@doLb,'interruptible','off');
ud.v.handles.rb = plot(ud.v.handles.gpa,[itw(end) itw(end)],[mi ma],'k',...
...
set(ud.v.handles.gpa,'xtick',xtick,'xticklabel',a);
set(ud.v.handles.axes,'xlim',[itw(1) itw(end)],'ylim',[ud.v.mi ud.v.ma]);

% Add temporal slider
ud.v.handles.hslider = uicontrol('parent',hp,'style','slider',...
    'units','normalized','Position',pos3,...
    'min',max([1,minSizeWindow/2-1]),'max',min([ud.v.nt,ud.v.nt-minSizeWindow/2+1]),...
...
for i=1:v.nc
    set(v.handles.hp(i),'xdata',xw(1):decim:xw(2),'ydata',y(i,:)+v.offset(i))
end
set(v.handles.axes,'ylim',[v.mi v.ma],'xlim',xw);
set(v.handles.pa,'xdata',[xw,fliplr(xw)]);
set(v.handles.lb,'xdata',[xw(1) xw(1)]);
set(v.handles.rb,'xdata',[xw(2) xw(2)]);
sw = diff(xw);
set(v.handles.hslider,'value',mean(xw),...
    'min',max([1,sw/2-1]),'max',max([v.nt,v.nt-sw/2+1]),...
