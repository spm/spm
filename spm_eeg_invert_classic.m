...
%     inverse.woi    - time window(s) of interest ([start stop] in ms)
...
switch smoothtype
    ...
    case 'mm_smooth'
         ...
         QG=asmth.QG;
         % What was this for? 'loc' does not exist in Dinv{val}.forward when I run
...
for w_idx=1:size(woi,1)
    ...
    VE(w_idx)     = sum(E(1:Nr));                  % variance explained
