...
tests = functiontests({@test_onesample_ttest_classical;
                       @test_onesample_ttest_bayesian;
                       @test_twosample_ttest_classical;
                       @test_twosample_ttest_bayesian});
...
% -------------------------------------------------------------------------
function test_onesample_ttest_bayesian(testCase)

xyz_mm = run_onesample_ttest(true);

% Check the global peak hasn't moved (3mm tolerance)
tol = [3 3 3]';
testCase.assertEqual(xyz_mm,[-32 -60 -24]','AbsTol', tol);
...
% -------------------------------------------------------------------------
function test_twosample_ttest_bayesian(testCase)

xyz_mm = run_twosample_ttest(true);
