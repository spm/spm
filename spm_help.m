...
% attribute
%
% This facility allows the sequential display of subroutines and their
% children by listing routines called by the current script.  These
% subroutines can be recursively examined along with their help page[s].
%
...
F    = figure('Color',[0.8 0.8 1],'Name','SPM95 Help facility',...
...
%----------------------------------------------------------------------------
c  = 'spm_sub(''spm_pet.man'');'  ;
uicontrol(F,'String','Overview',  'Position',[010 395 90 30]*A,'CallBack',c,'ForegroundColor',[1 0 0]);

c  = 'spm_sub(''spm_format.man'');'     ;
uicontrol(F,'String','Data format','Position',[110 395 90 30]*A,'CallBack',c,'ForegroundColor',COLOR);

c  = 'spm_sub(''spm_methods.man'');'    ;
uicontrol(F,'String','Methods',    'Position',[210 395 90 30]*A,'CallBack',c,'ForegroundColor',COLOR);
...
c  = 'spm_sub(''spm_F.man'');'          ;
uicontrol(F,'String','SPM{F}',     'Position',[040 130 070 30]*A,...
	'CallBack',c,          	   'Interruptible','yes');

c  = 'spm_sub(''spm_projections.man'');';
uicontrol(F,'String','SPM{Z}',     'Position',[120 130 070 30]*A,...
	'CallBack',c,		   'Interruptible','yes');

c  = 'spm_sub(''spm_sections.man'');'   ;
uicontrol(F,'String','Results',    'Position',[210 130 070 30]*A,...
	'CallBack',c,   	   'Interruptible','yes');

c  = 'spm_sub(''spm_surface_ui.m'');'       ;
uicontrol(F,'String','Render',     'Position',[290 130 070 30]*A,...
	'CallBack',c,    	   'Interruptible','yes');
...
c  = 'spm_sub(''spm_mri2pet.m'');'      ;
uicontrol(F,'String','mri to pet', 'Position',[300 56 80 24]*A,...
	'CallBack',c,              'Interruptible','yes');
