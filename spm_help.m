...
elseif strcmp(Topic,'Menu:Utils_PullDown')
	S = sprintf([...
		...
		'%s\n%%\n',...
		'%% 1) Analyze:   Executes `!analyze` in the base MatLab',...
			' workspace.\n',...
		'%%               Requires the `analyze` binary to be in',...
			' the path of the users shell.\n',...
		'%% 2) GhostView: Prompts for selection of a PostScript',...
			' file, and uses MatLab''s\n',...
		'%%               `unix` command to call `ghostscript` to',...
			' view it.\n',...
		'%%               Requires the `ghostview` binary to be in',...
			' the path of the users shell.\n',...
