...
covs = repeat('Covariates','covariates',{cov},'Covariates');
...
variance.val={1};
p1=['By default, the measurements in each level are assumed to have the same variance. '];
variance.help = {p1,sp_text};

dept    = mnu('Independence','dept',{'Yes','No'},{0,1},'');
dept.val={0};
p1=['By default, the measurements are assumed to be independent between levels. '];
dept.help = {p1,sp_text};
...
p1 = ['SPM will also automatically generate ',...
...
fac.help = {'Add a new factor to your design'};
...
p1=['Enter the numbers of the factors for this interaction.'];
fnums.help ={p1};
...
p1 = ['Create a design matrix a block at a time by specifying which ',...
      'main effects and interactions you wish to be included.'];
fblock.help={p1,sp_text};
...
im.val={1};
p1=['By default, an implicit mask is used. '];
im.help = {p1,sp_text};
...
aselect.val={0};
p1=['By default, Absolute Threshold Masking is turned off. '];
aselect.help = {p1,sp_text};
...
rselect.val={0};
p1=['By default, Relative Threshold Masking is turned off. '];
rselect.help = {p1,sp_text};
...
p2=['The interface configures the design matrix, describing the general ',...
    ...
    'configuration file (SPM.mat), and are ',...
